...
def exec_query(sql):
    ...
    cmd = "cat /tmp/temp_test.sql | docker exec -i supabase-db psql -U supabase_admin -d postgres -q -t -A"
    stdin, stdout, stderr = c.exec_command(cmd)
    out = stdout.read().decode("utf-8").strip()
    return out

print("=== 1. PREVIEW VS EXECUTION CONSISTENCY TEST ===")
...
for idx, tc in enumerate(test_cases):
    ...
    BEGIN
        ...
        RAISE NOTICE 'RESULT: team=% match=% status=%', v_team_id, v_match_type, v_status;
    ...
    # Execute and capture stderr for NOTICE
    s = c.open_sftp()
    f = s.file('/tmp/temp_test.sql', 'w')
    f.write(sql_test)
    f.close()
