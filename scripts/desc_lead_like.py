...
FROM crm_leads
WHERE id::text LIKE '0c11%';
"""
sftp.putfo(io.BytesIO(sql.encode()), "/tmp/desc_lead_like.sql")
_, stdout, stderr = c.exec_command("docker exec -i supabase-db psql -U supabase_admin -d postgres -c \"$(cat /tmp/desc_lead_like.sql)\"")
