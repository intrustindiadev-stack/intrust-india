...
BEGIN
    ...
    SELECT * INTO v_match FROM public.crm_match_team_for_location(NEW.pincode, NEW.zone, NEW.area, NEW.city, NEW.state);
...
sftp.putfo(io.BytesIO(sql.encode()), "/tmp/fix_routing2.sql")
_, stdout, stderr = c.exec_command("docker exec -i supabase-db psql -U supabase_admin -d postgres < /tmp/fix_routing2.sql")
