...
import { Clock, Mail, Home, ArrowRight, CheckCircle } from 'lucide-react';
...
export default function MerchantPendingPage() {
    ...
    // Realtime: auto-advance when admin approves the merchant
    useEffect(() => {
        let channel = null;
        ...
        supabase.auth.getSession().then(({ data: { session } }) => {
            ...
            channel = supabase
                .channel(`merchant-status:${session.user.id}`)
                .on(
                    'postgres_changes',
                    {
                        event: 'UPDATE',
                    ...
                    async (payload) => {
                        if (payload.new?.status === 'approved') {
                            setApproved(true);
                            // Refresh JWT so middleware picks up the new role
                            await supabase.auth.refreshSession();
                            router.replace('/merchant-subscribe');
                        }
                    }
                )
                .subscribe();
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-[#020617] flex items-center justify-center p-4 relative overflow-hidden transition-colors">
            ...
            <div className="max-w-2xl w-full merchant-glass rounded-3xl shadow-2xl overflow-hidden border border-black/5 dark:border-white/10">
                ...
                <div className="bg-gradient-to-br from-[#D4AF37] to-[#B8860B] dark:from-[#D4AF37]/20 dark:to-[#B8860B]/20 p-8 sm:p-12 text-center relative border-b border-black/5 dark:border-white/10">
                    ...
                    <h1 className="text-3xl sm:text-4xl font-display font-bold mb-3 text-slate-800 dark:text-white">
                        {approved ? 'Application Approved! 🎉' : 'Application Under Review'}
                    </h1>
                    <p className="text-slate-600 dark:text-slate-300 text-lg font-medium">
                        {approved ? 'Redirecting you to complete your subscription…' : "We're reviewing your merchant application"}
                    </p>
                </div>
                ...
                <div className="p-8 sm:p-12 space-y-8">
                    {approved ? (
                        <div className="flex items-start gap-4 p-5 bg-emerald-500/10 rounded-2xl border border-emerald-500/20">
                            ...
                            <div>
                                <p className="font-bold text-slate-800 dark:text-slate-100">You&apos;ve been approved!</p>
                                <p className="text-sm text-slate-600 dark:text-slate-400 font-medium">Taking you to your subscription page…</p>
                            </div>
                        </div>
                    ) : (
                        <>
