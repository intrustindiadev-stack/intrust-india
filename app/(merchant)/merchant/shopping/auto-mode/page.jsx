...
export default function AutoModePage() {
    ...
    useEffect(() => {
        ...
        const tick = () => {
            const diff = new Date(merchant.auto_mode_valid_until) - new Date();
            if (diff > 0) {
    ...
    const confirmDeactivation = async () => {
        ...
        try {
            ...
            setSuccess('Auto Mode deactivated.');
    ...
    const confirmActivation = async () => {
        if (walletBalance < subscriptionPrice) {
            setError(`Need ₹${subscriptionPrice} in wallet to activate.`);
            setTimeout(() => setError(null), 5000); return;
        }
        ...
        try {
            ...
            setMerchant(prev => ({ ...prev, auto_mode_status: 'active', auto_mode_months_paid: data.months_paid, auto_mode_valid_until: data.validUntil }));
            setSuccess('Auto Mode activated!');
