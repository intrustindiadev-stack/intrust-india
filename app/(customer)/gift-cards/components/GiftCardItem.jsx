...
import { Star, ShieldCheck, Heart, ChevronDown, Flame } from 'lucide-react';
import Link from 'next/link';
import { motion, AnimatePresence } from 'framer-motion';

export default function GiftCardItem({ coupon, index = 0, isExpanded = false, onToggle }) {
    ...
    return (
        ...
        >
            <div
                onClick={onToggle}
                className={`group relative bg-white rounded-3xl overflow-hidden border border-gray-100 hover:border-[#92BCEA]/50 transition-all duration-300 hover:shadow-2xl hover:shadow-[#92BCEA]/20 hover:-translate-y-1 h-full flex flex-col cursor-pointer ${isExpanded ? 'ring-2 ring-[#92BCEA]' : ''}`}
            >
                ...
                <div className={`relative h-48 bg-gradient-to-br ${coupon.gradient} overflow-hidden`}>
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 hidden sm:flex items-end justify-center pb-6">
                        <div className="bg-white text-gray-900 px-5 py-2.5 rounded-full font-bold flex items-center gap-2 shadow-xl hover:scale-105 transition-transform text-sm">
                ...
                <div className="p-5 flex-1 flex flex-col bg-gradient-to-b from-white to-gray-50/50">
                    ...
                    <AnimatePresence>
                        {isExpanded && (
                            ...
                            >
                                <div className="text-sm text-gray-600 bg-gray-50 rounded-xl p-3 border border-gray-100 leading-relaxed">
                                    {coupon.description}
                                </div>
                            </motion.div>
                        )}
                    ...
                    <Link
                        ...
                        className="w-full py-3.5 bg-gradient-to-r from-gray-900 to-gray-800 hover:from-[#92BCEA] hover:to-[#AFB3F7] text-white font-bold text-sm rounded-2xl transition-all duration-300 hover:shadow-xl hover:shadow-[#92BCEA]/30 flex items-center justify-center gap-2 group/btn mt-auto relative z-20"
                    >
                        <span>Buy Now</span>
