...
import { buildMerchantSystemInstruction } from '@/lib/chat/merchantPromptTemplates';

/**
 * POST /api/merchant/chat/message
 *
 * Merchant web chatbot backend — powered by Gemini AI (@google/genai SDK).
 * Supports multi-turn conversation via a `history` array sent by the client.
 * Enforces a role gate ('merchant', 'admin', 'super_admin') and scopes sessions
 * using the audience='merchant' discriminator.
 *
 * Request body:
 *   {
...
export async function POST(req) {
  try {
    ...
    try {
      ...
      if (!activeSessionId) {
        const { data: newSession, error: sessionErr } = await admin
          .from('webchat_sessions')
          .insert({ user_id: user.id, audience: 'merchant' })
          .select('id')
          .single();
        ...
        if (!providedSessionId) {
          try {
            await admin.from('notifications').insert({
              ...
              body: 'Your AI assistant is ready. Ask anything about your orders, payouts, or inventory.',
              type: 'success',
      ...
      console.warn('[merchant/chat/message] Failed to upsert session:', sessionErr.message || sessionErr);
    }

    // ── 3. Gemini API key check ──────────────────────────────────────────────
    if (!process.env.GEMINI_API_KEY) {
      console.error(
        '[merchant/chat/message] GEMINI_API_KEY is not set in environment variables. ' +
    ...
    const contextBlock = userCtx ? formatMerchantContextForPrompt(userCtx) : '';

    // ── 5. Build system instruction ──────────────────────────────────────────
    const systemInstruction = buildMerchantSystemInstruction(contextBlock);
    ...
    try {
      ...
      if (finishReason === 'MAX_TOKENS') {
        ...
        console.warn('[merchant/chat/message] Reply truncated by MAX_TOKENS', { length: rawText.length });
      ...
      try {
        if (activeSessionId) {
          await admin.from('webchat_messages').insert([
            { session_id: activeSessionId, user_id: user.id, role: 'user', content: userMessage, audience: 'merchant' },
            { session_id: activeSessionId, user_id: user.id, role: 'model', content: finalReply, audience: 'merchant' }
          ]);
        }
      } catch (msgLogErr) {
        console.warn('[merchant/chat/message] Failed to store messages in webchat_messages:', msgLogErr.message || msgLogErr);
      }

    } catch (aiErr) {
      console.error('[merchant/chat/message] Gemini call failed:', aiErr.message || aiErr);
    ...
    try {
      ...
      if (!_logTableMissingOnce) {
        ...
        console.warn(
          '[merchant/chat/message] Could not log to whatsapp_message_logs (table may not exist in dev):', 
    ...
    console.error('[merchant/chat/message] Unhandled error:', err);
    return NextResponse.json(
