    ...
    if (stuckTxns && stuckTxns.length > 0) {
        ...
        for (const txn of stuckTxns) {
            ...
            try {
                ...
                // 3c. Gateway says FAILED or ABORTED — close the transaction & cleanup order state
                if (internalStatus === 'failed' || internalStatus === 'aborted') {
                    ...
                    // Cleanup CART_CHECKOUT order group
                    if (txn.udf1 === 'CART_CHECKOUT' && txn.udf2) {
                        try {
                            await supabaseAdmin
                            ...
                            console.log(`[PaymentReconcile] Cart order ${txn.udf2} marked as cancelled/failed`);
                        } catch (cartErr) {
                            console.error(`[PaymentReconcile] Failed to update shopping order ${txn.udf2}:`, cartErr.message);
                        }
                    }

                    // Cleanup WHOLESALE_PURCHASE draft
                    if (txn.udf1 === 'WHOLESALE_PURCHASE' && txn.udf2) {
                        try {
                            await supabaseAdmin
                                .from('wholesale_order_drafts')
                                .update({
                                    status: 'failed',
                                    failure_reason: gatewayResult.message || 'Payment failed or aborted',
                                    updated_at: new Date().toISOString()
                                })
                                .eq('id', txn.udf2)
                                .neq('status', 'completed');
                            console.log(`[PaymentReconcile] Wholesale draft ${txn.udf2} marked as failed`);
                        } catch (whlsErr) {
                            console.error(`[PaymentReconcile] Failed to update wholesale draft ${txn.udf2}:`, whlsErr.message);
                        }
                    }
        ...
        console.log('[PaymentReconcile] No stuck transactions in 20m-24h window to check with SabPaisa.');
    }

    // 4. Auto-expire any stale pending gateway orders, wholesale drafts, and initiated transactions older than 30 minutes
    let expiredCleanup = null;
    try {
        const { data: expData, error: expErr } = await supabaseAdmin.rpc('expire_stale_gateway_orders', {
            p_timeout_minutes: 30
        });
        if (expErr) {
            console.error('[PaymentReconcile] Error running expire_stale_gateway_orders RPC:', expErr.message);
        } else {
            expiredCleanup = expData;
            console.log('[PaymentReconcile] expire_stale_gateway_orders result:', expData);
        }
    } catch (cleanErr) {
        console.error('[PaymentReconcile] Exception calling expire_stale_gateway_orders:', cleanErr.message);
    }

    const summary = {
        success: true,
        checked: stuckTxns ? stuckTxns.length : 0,
        fulfilled,
        failed,
        skipped,
        expiredCleanup,
