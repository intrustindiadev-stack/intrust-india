...
import { Loader2, AlertCircle, TrendingUp, Users, Target, CalendarDays, BarChart3 } from 'lucide-react';
...
export default function AnalyticsClient({ currentUserId, currentUserRole }) {
    ...
    const fetchAnalytics = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/crm/analytics?${params.toString()}`);
            if (!res.ok) {
    ...
    // Derive top-level metrics from server response
    const metrics = useMemo(() => {
        if (!analytics) return { total: 0, won: 0, active: 0, winRate: 0, revenue: 0, activeValue: 0 };
        ...
        return {
            ...
            winRate: t.win_rate,
            revenue: t.revenue,
            activeValue: t.active_value,
        };
    }, [analytics]);

    // Team leaderboard comes pre-sorted from server
    const teamLeaderboard = useMemo(() => {
        if (!isManager || !analytics?.team_leaderboard) return [];
        return analytics.team_leaderboard.map(m => ({
            id: m.id,
            name: m.name,
            avatar: m.avatar_url,
            totalLeads: m.total_assigned,
            wonLeads: m.deals_won,
            revenue: m.revenue,
            activePipeline: 0, // Not tracked in this view; available per-lead
            winRate: m.win_rate,
        }));
    }, [analytics, isManager]);

    // Chart data from monthly_trend — already bucketed server-side (year-aware)
                ...
                <div className="space-y-8">
                    ...
                    <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{delay: 0.4}} className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-[2.5rem] p-6 sm:p-8 border border-gray-200/50 dark:border-gray-700/50 shadow-2xl shadow-gray-200/20 dark:shadow-none">
                        <div className="flex items-center justify-between mb-8">
                            <h2 className="text-xl font-black text-gray-900 dark:text-white tracking-tight">Revenue Trends</h2>
                            <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">6-Month History</span>
                        </div>
                        <div className="h-[350px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                    ...
                                    <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#94a3b8', fontWeight: 'bold' }} tickFormatter={(v) => `₹${(v/1000).toFixed(0)}k`} />
                                    <Tooltip
                                            ...
                                            <div className="space-y-4 relative z-10">
                                                <div>
                                                    <div className="flex justify-between text-xs font-bold mb-1.5">
                                                        ...
                                                        <span className={Number(member.winRate) > 20 ? 'text-emerald-500' : 'text-indigo-500'}>{member.winRate}%</span>
                                                    </div>
                                                    <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-1.5 overflow-hidden">
                                                        <div className={`h-full rounded-full ${Number(member.winRate) > 20 ? 'bg-emerald-500' : 'bg-indigo-500'}`} style={{ width: `${Math.min(Number(member.winRate), 100)}%` }} />
                                                    </div>
                                                </div>
                                                ...
                                                <div className="pt-4 border-t border-gray-100 dark:border-gray-800 flex justify-between items-end">
                                                    ...
                                                    <div className="text-right">
                                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Won Deals</p>
                                                        <p className="text-sm font-bold text-gray-500 dark:text-gray-400">{member.wonLeads}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
