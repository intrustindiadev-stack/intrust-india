...
export default function CRMInvoicePage() {
    ...
    const handleGenerate = async () => {
        ...
        setGenerating(true);
        try {
            const mappedItems = items.map(item => ({ ...item, ...calculateItemTotal(item) }));
            const res = await fetch('/api/invoices', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ seller, customer, items: mappedItems, totals, meta: invoiceMeta })
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || 'Failed to save invoice');
            
            setGeneratedInvoiceData(data.invoice);
            
            generateManualInvoice({ seller, customer, items: mappedItems, totals, meta: { ...invoiceMeta, paymentUrl: data.invoice.payment_url } });
    ...
    const handleShareWhatsApp = () => {
        if (!generatedInvoiceData?.payment_url) return;
        const payableAmt = (generatedInvoiceData.grand_total_paise || 0) / 100;
        const text = `Dear ${customer.name || 'Customer'},\n\nPlease find your Intrust India Invoice #${generatedInvoiceData.invoice_number} for ₹${fmt(payableAmt)}.\n\nView and pay online here:\n${generatedInvoiceData.payment_url}\n\nThank you,\nIntrust India`;
    ...
    const handleShareEmail = () => {
        ...
        let url = generatedInvoiceData?.payment_url;
        const payableAmt = (generatedInvoiceData?.grand_total_paise || 0) / 100;
        const subject = `Invoice ${invoiceMeta.invoice_number} from ${seller.company_name}`;
        const body = `Dear ${customer.name},\n\nPlease find your invoice #${invoiceMeta.invoice_number} for ₹${fmt(payableAmt)}.\n\n${url ? `You can pay securely online here:\n${url}\n\n` : ''}Thank you for your business!`;
        window.open(`mailto:${customer.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`, '_blank');
    };

    const handleCopyLink = () => {
        if (generatedInvoiceData?.payment_url) {
            navigator.clipboard.writeText(generatedInvoiceData.payment_url);
    ...
    const handleDownloadAgain = () => {
        const mappedItems = items.map(item => ({ ...item, ...calculateItemTotal(item) }));
        generateManualInvoice({ seller, customer, items: mappedItems, totals, meta: { ...invoiceMeta, paymentUrl: generatedInvoiceData?.payment_url } });
    };
        ...
        <div className="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-10">
            <div className="max-w-5xl mx-auto space-y-6 pb-24">
                ...
                {generatedInvoiceData ? (
                    <div className="bg-white rounded-2xl shadow-sm border border-emerald-100 p-8 max-w-2xl mx-auto text-center space-y-6">
                        <div className="w-16 h-16 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-4">
                            <Receipt size={32} />
                        </div>
                        <h2 className="text-2xl font-black text-slate-900">Invoice Created ✓</h2>
                        ...
                        <div className="bg-slate-50 p-6 rounded-xl border border-slate-100 text-left space-y-3">
                            <div className="flex justify-between">
                                ...
                                <span className="font-bold text-slate-900">{generatedInvoiceData.invoice_number}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-slate-500 font-medium">Total Amount</span>
                                <span className="font-bold text-slate-900">₹{fmt(generatedInvoiceData.grand_total_paise / 100)}</span>
                            </div>
                            <div className="flex justify-between border-t border-slate-200 pt-3">
                                <span className="text-slate-500 font-medium">Amount Due</span>
                                <span className="font-black text-[#1e3a5f] text-lg">₹{fmt(generatedInvoiceData.grand_total_paise / 100)}</span>
                            </div>
                        </div>

                        <div className="text-left space-y-2">
                            <label className="block text-xs font-black text-slate-400 uppercase tracking-widest">Payment Link</label>
                            <div className="flex items-center gap-2">
                                <input type="text" readOnly value={generatedInvoiceData.payment_url} className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-medium text-slate-600 text-sm outline-none" />
                                <button onClick={handleCopyLink} className="px-4 py-3 bg-[#1e3a5f]/10 text-[#1e3a5f] hover:bg-[#1e3a5f] hover:text-white rounded-xl font-bold transition-all text-sm">
                                    Copy
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 pt-4">
                            <a href={generatedInvoiceData.payment_url} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 px-4 py-3 bg-white border-2 border-[#1e3a5f] text-[#1e3a5f] hover:bg-[#1e3a5f] hover:text-white rounded-xl font-bold transition-all text-sm">
                                Open Payment Page
                            </a>
                            <button onClick={handleDownloadAgain} className="flex items-center justify-center gap-2 px-4 py-3 bg-[#1e3a5f] text-white rounded-xl font-bold hover:bg-[#0f2447] transition-all text-sm">
                                <Download size={16} /> Download Invoice
                            </button>
                        </div>
                        
                        <div className="grid grid-cols-2 gap-3">
                            <button onClick={handleShareWhatsApp} disabled={!customer.phone} className="flex items-center justify-center gap-1.5 px-4 py-3 bg-green-50 text-green-700 border border-green-200 rounded-xl font-bold hover:bg-green-500 hover:text-white hover:border-green-500 transition-all text-sm disabled:opacity-40">
                                <Send size={16} /> Share via WhatsApp
                            </button>
                            <button onClick={handleShareEmail} disabled={!customer.email} className="flex items-center justify-center gap-1.5 px-4 py-3 bg-blue-50 text-blue-700 border border-blue-200 rounded-xl font-bold hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all text-sm disabled:opacity-40">
                                <Share2 size={16} /> Share via Email
                            </button>
                        </div>

                        <button onClick={() => { setGeneratedInvoiceData(null); setInvoiceMeta({...invoiceMeta, invoice_number: `CRM-${Date.now().toString(36).toUpperCase()}`}); }} className="mt-6 text-sm font-bold text-slate-400 hover:text-slate-600 underline">
