...
function AvatarUpload({ userId, avatarUrl, displayName, onUpload }) {
    ...
    const handleFileChange = async (e) => {
        ...
        try {
            ...
            const filePath = `${userId}/hrm_avatar.${ext}`;
    ...
    return (
        <div className="relative group mx-auto mb-10 w-fit">
            <div
                className="relative w-32 h-32 rounded-[2.5rem] overflow-hidden cursor-pointer shadow-2xl ring-4 ring-white/10 group-hover:ring-emerald-500/50 transition-all duration-500"
            ...
            >
                {avatarUrl ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-emerald-600/20 to-teal-600/20 flex items-center justify-center text-emerald-600 text-4xl font-black">
                        {initial}
                    </div>
                )}
            ...
            <div className="absolute -bottom-2 -right-2 w-10 h-10 rounded-2xl bg-emerald-600 text-white flex items-center justify-center shadow-xl border-4 border-white dark:border-[#020617] pointer-events-none transition-transform group-hover:scale-110">
                <Camera size={16} />
            </div>
...
function EditableRow({ label, value, icon: Icon, onSave, placeholder, readOnly = false }) {
    ...
    return (
        <div className="group flex items-start gap-5 py-5 border-b border-slate-100 dark:border-white/5 last:border-0">
            <div className={`mt-1 flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center ${readOnly ? 'bg-slate-100 dark:bg-white/5 text-slate-500' : 'bg-emerald-500/10 text-emerald-600'}`}>
                <Icon size={18} />
            </div>
                ...
                {editing ? (
                    <div className="flex items-center gap-2">
                        <input
                            ...
                            className="flex-1 text-sm bg-slate-50 dark:bg-white/5 border border-emerald-500 rounded-xl px-4 py-2 text-slate-900 dark:text-slate-100 outline-none ring-4 ring-emerald-500/10 min-w-0"
                        />
            ...
            {!readOnly && !editing && (
                <button
                    ...
                    className="mt-1 w-10 h-10 rounded-xl bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 flex items-center justify-center text-emerald-600 transition-all flex-shrink-0 border border-transparent hover:border-slate-200 dark:hover:border-white/10 shadow-sm active:scale-95"
                    title={`Edit ${label}`}
                >
                    <Edit2 size={14} />
                </button>
...
export default function HRMProfilePage() {
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
                <div className="relative w-16 h-16">
                    ...
                    <div className="absolute inset-2 rounded-full border-4 border-emerald-500/5 border-b-emerald-600 animate-spin-slow" />
                </div>
    ...
    const roleDisplay = profile.role ? profile.role.replace(/_/g, ' ') : 'HRM User';
            ...
            >
                <div className="space-y-2">
                    <div className="flex items-center gap-3">
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-900/5 text-emerald-600 text-[10px] font-black uppercase tracking-widest">
                            <User size={12} />
                            HRM Account
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-white/[0.03] backdrop-blur-3xl rounded-[2.5rem] border border-slate-100 dark:border-white/10 p-8 shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10 text-center">
                            ...
                            <h2 className="text-xl font-black text-slate-900 dark:text-slate-100 truncate mb-1">
                                {profile.full_name || 'HRM User'}
                            </h2>
                            <p className="text-emerald-600 font-black text-[10px] uppercase tracking-[0.2em] mb-6">
                                {roleDisplay}
                            </p>
                        </div>
                    </div>
