        ...
        <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-6 min-h-screen font-[family-name:var(--font-outfit)]">
            ...
            <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-slate-900">{employee.full_name}</h1>
                        <p className="text-slate-500 font-medium">{employee.department || employee.role}</p>
                    </div>
                </div>
                <div className="flex gap-4">
                    <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 min-w-[140px]">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Base Salary</p>
                        <p className="text-xl font-black text-slate-900 font-mono">₹{(employee.base_salary || 0).toLocaleString('en-IN')}</p>
                    </div>
                    <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-100 min-w-[140px]">
                        <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mb-1">YTD Earnings</p>
                        <p className="text-xl font-black text-emerald-700 font-mono">₹{ytdEarnings.toLocaleString('en-IN')}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Chart Section */}
                <div className="lg:col-span-2 bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6 sm:p-8">
                    <div className="flex items-center justify-between mb-8">
                        <h2 className="text-lg font-black text-slate-900">Earnings Trend</h2>
                    ...
                    <div className="h-72 w-full">
                        {chartData.length > 0 ? (
                            ...
                            <div className="h-full flex items-center justify-center text-slate-400 font-medium">No payroll history available</div>
                        )}
                    </div>
                </div>

                {/* History List */}
                <div className="lg:col-span-1 bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6 sm:p-8 flex flex-col">
                    <h2 className="text-lg font-black text-slate-900 mb-6">Payroll History</h2>
                    <div className="flex-1 overflow-y-auto pr-2 space-y-4 hide-scrollbar">
                        {salaryRecords.length === 0 ? (
                            ...
                            salaryRecords.map(sal => (
                                <div key={sal.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-slate-200 transition-colors group">
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        {sal.status === 'processed' ? (
                                            <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-100 px-2 py-0.5 rounded-md"><CheckCircle2 size={12} /> Paid</span>
                                        ) : (
                                            <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-amber-600 bg-amber-100 px-2 py-0.5 rounded-md"><AlertCircle size={12} /> Pending</span>
                                        )}
                                    </div>
                                    <div className="flex justify-between items-end mt-4">
                                        <div>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Net Pay</p>
                                            <p className="font-black text-slate-900 font-mono text-lg">₹{(sal.net_salary || 0).toLocaleString('en-IN')}</p>
                                        </div>
                                        {sal.status === 'processed' && (
                                            <button 
                                                onClick={() => handleDownload(sal)}
                                                className="w-8 h-8 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 transition-all"
                                            >
                                                <Download size={14} />
                                            </button>
                                        )}
