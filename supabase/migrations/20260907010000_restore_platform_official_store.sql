-- Migration: 20260907010000_restore_platform_official_store.sql
-- Description: Restore platform Official Store architecture backed by shopping_products (platform_listed = true)
-- and preserve separate merchant architecture for actual merchants (backed by merchant_inventory).
...
AS $function$
 ...
 BEGIN
     ...
     -- =========================================================================
     -- 1. PLATFORM OFFICIAL STORE BRANCH
     -- Backed by centralized shopping_products table where platform_listed = true
     -- =========================================================================
     IF v_norm_slug = 'official' THEN
         ...
         WHERE sp.platform_listed = true
           ...
           AND (COALESCE(p_search, '') = '' OR sp.title ILIKE '%' || p_search || '%' OR sp.description ILIKE '%' || p_search || '%')
           AND (COALESCE(p_brand, '') = '' OR sp.title ILIKE '%' || p_brand || '%')
         ...
         -- Fetch paginated platform products
         FOR v_item IN
             SELECT
                 sp.id,
                 sp.title,
     ...
     -- =========================================================================
     -- 2. ACTUAL MERCHANT STOREFRONT BRANCH
     -- Backed by real merchant record & merchant_inventory
     -- =========================================================================
     ELSE
         -- Fetch merchant
...
-- 2. Update get_merchant_categories RPC
...
BEGIN
    ...
    IF v_norm_slug = 'official' THEN
        SELECT array_agg(DISTINCT category ORDER BY category ASC) INTO v_categories
    ...
    ELSE
        ...
        IF FOUND THEN
            SELECT array_agg(DISTINCT sp.category ORDER BY sp.category ASC) INTO v_categories
            FROM public.merchant_inventory mi
            JOIN public.shopping_products sp ON mi.product_id = sp.id
            WHERE mi.merchant_id = v_merchant_id
              AND mi.is_active = true
