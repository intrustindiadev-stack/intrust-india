-- =============================================================================
-- Migration: 20260925000000_atomic_ai_grow_settlement.sql
-- Description:
--   Creates atomic, idempotent PostgreSQL RPC functions for AI Grow settlements:
--   1. public.settle_ai_grow_investment(...)
--      - Atomic settlement of an individual AI Grow growth plan.
--      - Supports 'wallet' (principal + profit credited to merchant wallet)
--        and 'offline_cash' (marked paid offline; 0 wallet credit).
--      - Executes vault debit, wallet bypass credit, ledger updates, status change,
--        notifications, and audit logging in a SINGLE database transaction.
--   2. public.settle_ai_grow_vault_to_wallet(...)
--      - Atomic liquidation of arbitrary available AI Grow vault capital directly
--        to the merchant's active wallet balance.
-- =============================================================================

BEGIN;

-- -----------------------------------------------------------------------------
-- 1. Settle AI Grow Investment RPC
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.settle_ai_grow_investment(
    p_investment_id UUID,
    p_admin_id UUID,
    p_settlement_destination TEXT,
    p_idempotency_key TEXT DEFAULT NULL,
    p_notes TEXT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
DECLARE
    v_investment RECORD;
    v_merchant RECORD;
    v_ai_wallet RECORD;
    v_principal_paise BIGINT;
    v_profit_paise BIGINT := 0;
    v_total_payout_paise BIGINT;
    v_principal_rupees NUMERIC(14,2);
    v_merchant_wallet_before BIGINT;
    v_merchant_wallet_after BIGINT;
    v_vault_before NUMERIC(14,2);
    v_vault_after NUMERIC(14,2);
    v_vault_tx_id UUID;
    v_merchant_tx_id UUID;
    v_is_super_admin BOOLEAN := FALSE;
    v_caller_role TEXT;
    v_admin_role_val user_role := 'super_admin'::user_role;
BEGIN
    -- 1. Validate destination
    IF p_settlement_destination NOT IN ('wallet', 'offline_cash') THEN
        RAISE EXCEPTION 'Invalid settlement destination: %. Must be "wallet" or "offline_cash".', p_settlement_destination
            USING ERRCODE = '22023';
    END IF;

    -- 2. Validate caller authorization
    -- Super admin role is strictly required for settling growth plans.
    IF p_admin_id IS NOT NULL THEN
        SELECT EXISTS (
            SELECT 1 FROM public.user_profiles
            WHERE id = p_admin_id AND role = 'super_admin'
        ) INTO v_is_super_admin;

        IF NOT v_is_super_admin THEN
            RAISE EXCEPTION 'Access denied. Super admin role required for AI Grow settlement.'
                USING ERRCODE = '42501';
        END IF;
    ELSE
        v_caller_role := current_setting('role', true);
        IF NOT (v_caller_role IN ('service_role', 'supabase_admin') OR session_user IN ('postgres', 'supabase_admin')) THEN
            RAISE EXCEPTION 'Unauthorized: Super admin identity required for AI Grow settlement.'
                USING ERRCODE = '42501';
        END IF;
    END IF;

    -- 3. Lock and retrieve the investment row
    SELECT * INTO v_investment
    FROM public.merchant_investments
    WHERE id = p_investment_id
    FOR UPDATE;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'AI Grow investment % not found.', p_investment_id
            USING ERRCODE = 'P0002';
    END IF;

    -- 4. Idempotency Check: check if already settled
    IF v_investment.status IN ('completed', 'released') THEN
        RETURN jsonb_build_object(
            'success', false,
            'already_settled', true,
            'error', 'Investment has already been settled (status: ' || v_investment.status || ').',
            'investment_id', p_investment_id,
            'status', v_investment.status
        );
    END IF;

    IF v_investment.status <> 'active' THEN
        RAISE EXCEPTION 'Investment cannot be settled. Current status is "%", but must be "active".', v_investment.status
            USING ERRCODE = '55000';
    END IF;

    -- 5. Lock and retrieve the merchant row
    SELECT * INTO v_merchant
    FROM public.merchants
    WHERE id = v_investment.merchant_id
    FOR UPDATE;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'Merchant % not found.', v_investment.merchant_id
            USING ERRCODE = 'P0002';
    END IF;

    -- 6. Lock and retrieve the AI Grow vault wallet row
    SELECT * INTO v_ai_wallet
    FROM public.ai_grow_wallets
    WHERE merchant_id = v_investment.merchant_id
    FOR UPDATE;

    IF NOT FOUND THEN
        -- Auto-provision vault wallet row if it predated the ledger
        INSERT INTO public.ai_grow_wallets (merchant_id, balance)
        VALUES (v_investment.merchant_id, 0.00)
        RETURNING * INTO v_ai_wallet;
    END IF;

    -- 7. Calculate Principal, Profit and Total Payout (PAISE ARITHMETIC)
    v_principal_paise := v_investment.amount_paise;
    v_principal_rupees := ROUND((v_principal_paise::numeric / 100.0), 2);

    SELECT COALESCE(SUM(profit_paise), 0)
    INTO v_profit_paise
    FROM public.merchant_investment_orders
    WHERE investment_id = p_investment_id;

    v_total_payout_paise := v_principal_paise + v_profit_paise;

    IF v_principal_paise <= 0 THEN
        RAISE EXCEPTION 'Invalid investment principal: % paise.', v_principal_paise
            USING ERRCODE = '22023';
    END IF;

    -- Check vault balance
    IF v_ai_wallet.balance < v_principal_rupees THEN
        RAISE EXCEPTION 'Insufficient AI Grow vault balance: available ₹%, required ₹% for principal exit.',
            v_ai_wallet.balance, v_principal_rupees
            USING ERRCODE = '22003';
    END IF;

    v_vault_before := v_ai_wallet.balance;
    v_vault_after := v_vault_before - v_principal_rupees;
    v_merchant_wallet_before := COALESCE(v_merchant.wallet_balance_paise, 0);

    -- 8. Debit AI Grow Vault (Principal exits the AI Grow system)
    UPDATE public.ai_grow_wallets
    SET balance = v_vault_after,
        updated_at = NOW()
    WHERE id = v_ai_wallet.id;

    INSERT INTO public.ai_grow_wallet_transactions (
        wallet_id,
        merchant_id,
        admin_id,
        transaction_type,
        amount,
        previous_balance,
        new_balance,
        reason,
        metadata
    ) VALUES (
        v_ai_wallet.id,
        v_merchant.id,
        p_admin_id,
        'debit',
        v_principal_rupees,
        v_vault_before,
        v_vault_after,
        'AI Grow principal exit via ' || p_settlement_destination || ' settlement for investment ' || p_investment_id::text,
        jsonb_build_object(
            'investment_id', p_investment_id,
            'settlement_destination', p_settlement_destination,
            'principal_paise', v_principal_paise,
            'profit_paise', v_profit_paise,
            'total_payout_paise', v_total_payout_paise,
            'idempotency_key', p_idempotency_key,
            'notes', p_notes,
            'settled_at', NOW()
        )
    ) RETURNING id INTO v_vault_tx_id;

    -- 9. Handle Settlement Destination
    IF p_settlement_destination = 'wallet' THEN
        v_merchant_wallet_after := v_merchant_wallet_before + v_total_payout_paise;

        -- Set app.internal_bypass for protected column update
        PERFORM set_config('app.internal_bypass', 'true', true);

        UPDATE public.merchants
        SET wallet_balance_paise = v_merchant_wallet_after,
            updated_at = NOW()
        WHERE id = v_merchant.id;

        PERFORM set_config('app.internal_bypass', 'false', true);

        -- Insert into merchant_transactions ledger
        INSERT INTO public.merchant_transactions (
            merchant_id,
            transaction_type,
            amount_paise,
            balance_after_paise,
            description,
            metadata
        ) VALUES (
            v_merchant.id,
            'wallet_topup',
            v_total_payout_paise,
            v_merchant_wallet_after,
            'AI Grow Capital + Profit Settled to Wallet',
            jsonb_build_object(
                'source', 'AI_GROW',
                'investment_id', p_investment_id,
                'settlement_destination', 'wallet',
                'principal_paise', v_principal_paise,
                'profit_paise', v_profit_paise,
                'total_payout_paise', v_total_payout_paise,
                'vault_transaction_id', v_vault_tx_id,
                'admin_id', p_admin_id,
                'idempotency_key', p_idempotency_key,
                'notes', p_notes,
                'settled_at', NOW()
            )
        ) RETURNING id INTO v_merchant_tx_id;

    ELSIF p_settlement_destination = 'offline_cash' THEN
        v_merchant_wallet_after := v_merchant_wallet_before; -- NO CHANGE TO DIGITAL WALLET
        v_merchant_tx_id := NULL;
    END IF;

    -- 10. Mark investment status = 'completed'
    UPDATE public.merchant_investments
    SET status = 'completed',
        admin_id = COALESCE(p_admin_id, v_investment.admin_id),
        updated_at = NOW()
    WHERE id = p_investment_id;

    -- 11. Send in-app notification to merchant
    IF v_merchant.user_id IS NOT NULL THEN
        IF p_settlement_destination = 'wallet' THEN
            INSERT INTO public.notifications (
                user_id,
                title,
                body,
                type,
                reference_id,
                reference_type,
                metadata
            ) VALUES (
                v_merchant.user_id,
                'AI Grow Growth Plan Settled to Wallet',
                'Your AI Grow growth plan of ₹' || (v_total_payout_paise::numeric / 100.0)::text || ' (Principal ₹' || (v_principal_paise::numeric / 100.0)::text || ' + Profit ₹' || (v_profit_paise::numeric / 100.0)::text || ') has been credited to your merchant wallet.',
                'success',
                p_investment_id,
                'investment',
                jsonb_build_object(
                    'destination', 'wallet',
                    'total_payout_paise', v_total_payout_paise,
                    'principal_paise', v_principal_paise,
                    'profit_paise', v_profit_paise,
                    'wallet_after_paise', v_merchant_wallet_after
                )
            );
        ELSE
            INSERT INTO public.notifications (
                user_id,
                title,
                body,
                type,
                reference_id,
                reference_type,
                metadata
            ) VALUES (
                v_merchant.user_id,
                'AI Grow Growth Plan Paid Offline',
                'Your AI Grow growth plan of ₹' || (v_total_payout_paise::numeric / 100.0)::text || ' (Principal ₹' || (v_principal_paise::numeric / 100.0)::text || ' + Profit ₹' || (v_profit_paise::numeric / 100.0)::text || ') has been marked as paid offline.',
                'info',
                p_investment_id,
                'investment',
                jsonb_build_object(
                    'destination', 'offline_cash',
                    'total_payout_paise', v_total_payout_paise,
                    'principal_paise', v_principal_paise,
                    'profit_paise', v_profit_paise,
                    'wallet_credit_paise', 0
                )
            );
        END IF;
    END IF;

    -- 12. Insert Audit Log
    INSERT INTO public.audit_logs (
        actor_id,
        actor_role,
        action,
        entity_type,
        entity_id,
        description,
        metadata
    ) VALUES (
        p_admin_id,
        v_admin_role_val,
        'admin_action'::audit_action,
        'ai_grow_investment_settlement',
        p_investment_id,
        CASE 
            WHEN p_settlement_destination = 'wallet' THEN
                'Settled AI Grow investment to merchant wallet: ₹' || (v_total_payout_paise::numeric / 100.0)::text || ' (Principal ₹' || (v_principal_paise::numeric / 100.0)::text || ' + Profit ₹' || (v_profit_paise::numeric / 100.0)::text || ') for ' || COALESCE(v_merchant.business_name, 'Merchant')
            ELSE
                'Marked AI Grow investment as paid offline (cash): ₹' || (v_total_payout_paise::numeric / 100.0)::text || ' for ' || COALESCE(v_merchant.business_name, 'Merchant') || ' (Digital wallet credited ₹0)'
        END,
        jsonb_build_object(
            'investment_id', p_investment_id,
            'merchant_id', v_merchant.id,
            'destination', p_settlement_destination,
            'principal_paise', v_principal_paise,
            'profit_paise', v_profit_paise,
            'total_payout_paise', v_total_payout_paise,
            'vault_before', v_vault_before,
            'vault_after', v_vault_after,
            'wallet_before_paise', v_merchant_wallet_before,
            'wallet_after_paise', v_merchant_wallet_after,
            'vault_tx_id', v_vault_tx_id,
            'merchant_tx_id', v_merchant_tx_id,
            'idempotency_key', p_idempotency_key,
            'notes', p_notes
        )
    );

    -- 13. Return complete result
    RETURN jsonb_build_object(
        'success', true,
        'investment_id', p_investment_id,
        'merchant_id', v_merchant.id,
        'destination', p_settlement_destination,
        'principal_paise', v_principal_paise,
        'profit_paise', v_profit_paise,
        'total_payout_paise', v_total_payout_paise,
        'vault_before', v_vault_before,
        'vault_after', v_vault_after,
        'wallet_before_paise', v_merchant_wallet_before,
        'wallet_after_paise', v_merchant_wallet_after,
        'vault_transaction_id', v_vault_tx_id,
        'merchant_transaction_id', v_merchant_tx_id,
        'settled_at', NOW()
    );
END;
$$;

-- Revoke public access, allow authenticated and service_role
REVOKE ALL ON FUNCTION public.settle_ai_grow_investment FROM PUBLIC;
REVOKE ALL ON FUNCTION public.settle_ai_grow_investment FROM anon;
GRANT EXECUTE ON FUNCTION public.settle_ai_grow_investment TO authenticated;
GRANT EXECUTE ON FUNCTION public.settle_ai_grow_investment TO service_role;


-- -----------------------------------------------------------------------------
-- 2. Settle AI Grow Vault to Wallet RPC
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.settle_ai_grow_vault_to_wallet(
    p_merchant_id UUID,
    p_admin_id UUID,
    p_amount_rupees NUMERIC,
    p_reason TEXT DEFAULT 'AI Grow Vault Settlement to Merchant Wallet',
    p_settlement_type TEXT DEFAULT 'Full Principal Liquidation',
    p_idempotency_key TEXT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
DECLARE
    v_merchant RECORD;
    v_ai_wallet RECORD;
    v_amount_paise BIGINT;
    v_wallet_before BIGINT;
    v_wallet_after BIGINT;
    v_vault_before NUMERIC(14,2);
    v_vault_after NUMERIC(14,2);
    v_vault_tx_id UUID;
    v_merchant_tx_id UUID;
    v_is_super_admin BOOLEAN := FALSE;
    v_caller_role TEXT;
    v_admin_role_val user_role := 'super_admin'::user_role;
BEGIN
    -- 1. Validate caller authorization (super_admin)
    IF p_admin_id IS NOT NULL THEN
        SELECT EXISTS (
            SELECT 1 FROM public.user_profiles
            WHERE id = p_admin_id AND role = 'super_admin'
        ) INTO v_is_super_admin;

        IF NOT v_is_super_admin THEN
            RAISE EXCEPTION 'Access denied. Super admin role required for AI Grow vault settlement.'
                USING ERRCODE = '42501';
        END IF;
    ELSE
        v_caller_role := current_setting('role', true);
        IF NOT (v_caller_role IN ('service_role', 'supabase_admin') OR session_user IN ('postgres', 'supabase_admin')) THEN
            RAISE EXCEPTION 'Unauthorized: Super admin identity required for AI Grow vault settlement.'
                USING ERRCODE = '42501';
        END IF;
    END IF;

    -- 2. Validate amount
    IF p_amount_rupees <= 0 THEN
        RAISE EXCEPTION 'Settlement amount must be greater than zero.' USING ERRCODE = '22023';
    END IF;

    v_amount_paise := ROUND(p_amount_rupees * 100);

    -- 3. Lock merchant row
    SELECT * INTO v_merchant
    FROM public.merchants
    WHERE id = p_merchant_id
    FOR UPDATE;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'Merchant % not found.', p_merchant_id USING ERRCODE = 'P0002';
    END IF;

    -- 4. Lock AI Grow wallet row
    SELECT * INTO v_ai_wallet
    FROM public.ai_grow_wallets
    WHERE merchant_id = p_merchant_id
    FOR UPDATE;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'AI Grow vault not found for merchant %.', p_merchant_id USING ERRCODE = 'P0002';
    END IF;

    IF v_ai_wallet.status <> 'active' THEN
        RAISE EXCEPTION 'AI Grow vault is %. Settlements are only permitted on active vaults.', v_ai_wallet.status USING ERRCODE = '55000';
    END IF;

    IF v_ai_wallet.balance < p_amount_rupees THEN
        RAISE EXCEPTION 'Settlement amount (₹%) exceeds current AI Grow vault balance (₹%).', p_amount_rupees, v_ai_wallet.balance USING ERRCODE = '22003';
    END IF;

    v_vault_before := v_ai_wallet.balance;
    v_vault_after := v_vault_before - p_amount_rupees;
    v_wallet_before := COALESCE(v_merchant.wallet_balance_paise, 0);
    v_wallet_after := v_wallet_before + v_amount_paise;

    -- 5. Debit AI Grow vault
    UPDATE public.ai_grow_wallets
    SET balance = v_vault_after,
        updated_at = NOW()
    WHERE id = v_ai_wallet.id;

    INSERT INTO public.ai_grow_wallet_transactions (
        wallet_id,
        merchant_id,
        admin_id,
        transaction_type,
        amount,
        previous_balance,
        new_balance,
        reason,
        metadata
    ) VALUES (
        v_ai_wallet.id,
        v_merchant.id,
        p_admin_id,
        'debit',
        p_amount_rupees,
        v_vault_before,
        v_vault_after,
        'Vault Settlement to Wallet: ' || p_settlement_type || '. ' || p_reason,
        jsonb_build_object(
            'action', 'vault_settlement_to_wallet',
            'settlement_type', p_settlement_type,
            'idempotency_key', p_idempotency_key,
            'amount_paise', v_amount_paise,
            'settled_at', NOW()
        )
    ) RETURNING id INTO v_vault_tx_id;

    -- 6. Credit Merchant Wallet using app.internal_bypass
    PERFORM set_config('app.internal_bypass', 'true', true);

    UPDATE public.merchants
    SET wallet_balance_paise = v_wallet_after,
        updated_at = NOW()
    WHERE id = v_merchant.id;

    PERFORM set_config('app.internal_bypass', 'false', true);

    -- 7. Insert into merchant_transactions ledger
    INSERT INTO public.merchant_transactions (
        merchant_id,
        transaction_type,
        amount_paise,
        balance_after_paise,
        description,
        metadata
    ) VALUES (
        v_merchant.id,
        'wallet_topup',
        v_amount_paise,
        v_wallet_after,
        'AI Grow Vault Settlement: ₹' || p_amount_rupees::text,
        jsonb_build_object(
            'source', 'ai_grow_vault',
            'settlement_type', p_settlement_type,
            'reason', p_reason,
            'vault_transaction_id', v_vault_tx_id,
            'admin_id', p_admin_id,
            'idempotency_key', p_idempotency_key,
            'settled_at', NOW()
        )
    ) RETURNING id INTO v_merchant_tx_id;

    -- 8. Send notification
    IF v_merchant.user_id IS NOT NULL THEN
        INSERT INTO public.notifications (
            user_id,
            title,
            body,
            type,
            reference_id,
            reference_type,
            metadata
        ) VALUES (
            v_merchant.user_id,
            'AI Grow Vault Funds Settled',
            '₹' || p_amount_rupees::text || ' from your AI Grow Vault has been settled into your active Merchant Wallet.',
            'success',
            v_vault_tx_id,
            'vault_settlement',
            jsonb_build_object(
                'amount_paise', v_amount_paise,
                'wallet_after_paise', v_wallet_after
            )
        );
    END IF;

    -- 9. Insert audit log
    INSERT INTO public.audit_logs (
        actor_id,
        actor_role,
        action,
        entity_type,
        entity_id,
        description,
        metadata
    ) VALUES (
        p_admin_id,
        v_admin_role_val,
        'admin_action'::audit_action,
        'ai_grow_vault_settlement',
        p_merchant_id,
        'Settled ₹' || p_amount_rupees::text || ' from AI Grow Vault to Merchant Wallet for ' || COALESCE(v_merchant.business_name, 'Merchant'),
        jsonb_build_object(
            'merchant_id', p_merchant_id,
            'amount_rupees', p_amount_rupees,
            'amount_paise', v_amount_paise,
            'vault_before', v_vault_before,
            'vault_after', v_vault_after,
            'wallet_before_paise', v_wallet_before,
            'wallet_after_paise', v_wallet_after,
            'vault_tx_id', v_vault_tx_id,
            'merchant_tx_id', v_merchant_tx_id
        )
    );

    RETURN jsonb_build_object(
        'success', true,
        'merchant_id', p_merchant_id,
        'settled_amount_rupees', p_amount_rupees,
        'settled_amount_paise', v_amount_paise,
        'vault_before', v_vault_before,
        'vault_after', v_vault_after,
        'wallet_before_paise', v_wallet_before,
        'wallet_after_paise', v_wallet_after,
        'vault_transaction_id', v_vault_tx_id,
        'merchant_transaction_id', v_merchant_tx_id
    );
END;
$$;

-- Revoke public access, allow authenticated and service_role
REVOKE ALL ON FUNCTION public.settle_ai_grow_vault_to_wallet FROM PUBLIC;
REVOKE ALL ON FUNCTION public.settle_ai_grow_vault_to_wallet FROM anon;
GRANT EXECUTE ON FUNCTION public.settle_ai_grow_vault_to_wallet TO authenticated;
GRANT EXECUTE ON FUNCTION public.settle_ai_grow_vault_to_wallet TO service_role;

COMMIT;
