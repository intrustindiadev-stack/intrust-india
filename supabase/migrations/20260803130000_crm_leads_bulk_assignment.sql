-- Migration: CRM Leads Bulk Assignment & Role Cleanup
-- Date: 2026-08-03

-- 1. Create a bulk assignment RPC
CREATE OR REPLACE FUNCTION public.crm_bulk_assign_leads(
    p_lead_ids   UUID[],
    p_new_rep_id UUID
)
RETURNS JSONB
...
BEGIN
    ...
    INSERT INTO public.audit_logs_crm (actor_id, action, table_name, record_id, metadata)
    VALUES (
        v_caller_id,
        'BULK_ASSIGNMENT',
        'crm_leads',
...
-- 2. Add useful indexes for CRM filtering & sorting if they do not exist
CREATE INDEX IF NOT EXISTS idx_crm_leads_assigned_to_created_at ON public.crm_leads (assigned_to, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_crm_leads_status_created_at ON public.crm_leads (status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_crm_leads_source_created_at ON public.crm_leads (source, created_at DESC);
