-- Add platform_listed parameter to product creation RPCs

DROP FUNCTION IF EXISTS public.admin_insert_shopping_product(text,text,text,uuid,bigint,bigint,bigint,integer,text[],boolean,integer,text);
DROP FUNCTION IF EXISTS public.admin_update_shopping_product(uuid,text,text,text,uuid,bigint,bigint,bigint,integer,text[],boolean,integer,text);
...
BEGIN
    ...
    INSERT INTO public.shopping_products (
        ...
        mrp_paise, admin_stock, product_images, is_active, platform_listed, gst_percentage, hsn_code
    ) VALUES (
        p_title, p_description, p_category, p_category_id, p_wholesale_price, p_retail_price,
        p_mrp_paise, p_admin_stock, p_product_images, p_is_active, p_platform_listed, p_gst_percentage, p_hsn_code
    )
