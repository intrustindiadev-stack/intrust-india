-- ============================================================================
-- FIX: Daily Login Reward Point Increase and Scratch Card Bypass
-- Date: 2026-05-09
-- Description:
--   1. Ensures daily_login_reward is set to 50 points.
--   2. Updates calculate_and_distribute_rewards to automatically mark 
--      daily_login rewards as "scratched" so they bypass the scratch card UI.
--   3. Backfills existing daily_login rewards to be marked as scratched.
-- ============================================================================

-- 1. Ensure 50 points for daily login
UPDATE public.reward_configuration
SET config_value = config_value || '{"direct": 50}'::jsonb
WHERE config_key = 'daily_login_reward';

-- 2. Update RPC to auto-scratch daily login rewards
...
BEGIN
    ...
    LOOP
        ...
        -- 7. For purchase events, calculate based on amount
...
-- 3. Backfill existing daily_login rewards to be marked as scratched
UPDATE public.reward_transactions
SET is_scratched = true
WHERE event_type = 'daily_login' AND is_scratched = false;
