-- ============================================================
-- INTRUST INDIA — Fix P7 Regression in draft_cart_orders
-- Migration: 20260824000000_fix_draft_cart_orders_p7_regression.sql
--
-- Addresses:
--   - P7 rewrite introduced a regression replacing `customer_id` with `user_id`
--     in shopping_cart, causing 42703 undefined column.
--   - P7 omitted total_amount_paise and payment_status in shopping_order_groups INSERT,
--     causing 23502 NOT NULL violation.
--   - P7 omitted product_id, cost_price_paise, profit_paise in shopping_order_items.
--   - P7 removed delivery fee, GST, and address logic.
--
-- Solution:
--   - Restores the full known-good implementation from 20260609000000
--   - Preserves the P7 IDOR protection (auth.uid() override).
-- ============================================================
...
BEGIN
    ...
    -- Ignore p_customer_id entirely for authenticated users to prevent IDOR.
    v_caller_uid := auth.uid();

    IF v_caller_uid IS NULL THEN
        -- service_role calls (e.g. server-side cart seeding)
        IF p_customer_id IS NULL THEN
            RAISE EXCEPTION 'Caller identity could not be determined.';
        END IF;
        v_customer_id := p_customer_id;
    ELSE
        -- Regular authenticated user: ALWAYS use auth.uid()
    ...
    INSERT INTO public.shopping_order_groups (
        ...
        delivery_fee_paise, payment_method, payment_status
    )
    VALUES (
        ...
        v_delivery_fee_paise, 'gateway', 'pending'
    )
    ...
    LOOP
        IF v_item.is_platform_item AND v_item.merchant_id IS NULL THEN
            INSERT INTO public.shopping_order_items (
                ...
                v_item.effective_price, COALESCE(v_item.platform_cost, 0),
...
DO $$
BEGIN
    RAISE NOTICE 'Fix applied: draft_cart_orders restored to pre-P7 schema with IDOR protection.';
END;
$$;
