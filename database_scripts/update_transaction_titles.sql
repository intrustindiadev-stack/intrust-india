...
BEGIN
    ...
    -- 2. Get Delivery Address from profile
    SELECT address INTO v_delivery_address
    FROM public.user_profiles
    WHERE id = p_customer_id;

    -- Fallback to KYC address if profile address is empty
    ...
    -- 3. Identify Merchant/Platform and Validate Stock
    ...
    -- 4. CREATE ORDER GROUP with persistent delivery address and fee
    ...
    VALUES (
        p_customer_id, v_total_paise, 'completed', 'pending', v_merchant_id, v_is_platform, v_delivery_address, v_delivery_fee_paise, 'wallet'
    )
    ...
    LOOP
        ...
        IF v_item.is_platform_item THEN
            -- PLATFORM PRODUCT: Full profit goes to admin
        ...
        ELSE
            ...
            SET wallet_balance_paise = wallet_balance_paise + v_merchant_credit,
    ...
    INSERT INTO public.customer_wallet_transactions (
        ...
        p_customer_id, 'DEBIT', v_total_paise, v_wallet_balance, v_wallet_balance - v_total_paise,
