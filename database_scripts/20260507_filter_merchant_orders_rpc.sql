-- Migration: Filter merchant orders RPC — hide drafts / failed / pending_credit
-- Date: 2026-05-07
--
-- Changes from 20260423_add_payment_method_to_merchant_orders_rpc.sql:
--   • Excludes abandoned Sabpaisa gateway drafts (payment_method='gateway' AND payment_status='pending')
--   • Excludes failed / cancelled order groups (status IN ('failed','cancelled'))
--   • Excludes store-credit orders awaiting merchant approval (delivery_status='pending_credit')
--     because they are already surfaced in StoreCreditRequestsTab via udhari_requests.
-- The function signature, JSON shape, and auth check are identical to the previous version.
...
BEGIN
    ...
    WHERE og.merchant_id = p_merchant_id
      ...
      AND NOT (og.payment_method = 'gateway' AND og.payment_status = 'pending')
...
-- Re-grant explicitly to ensure authenticated role retains EXECUTE permission
