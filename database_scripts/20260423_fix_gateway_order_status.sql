-- Part A: Fix finalize_gateway_orders RPC
-- Derived from update_finalize_gateway_orders_v2.sql with payment_status fix
...
-- Part B: Backfill the stuck order(s)
-- Safely target only gateway orders that were paid but never transitioned to completed
UPDATE public.shopping_order_groups
