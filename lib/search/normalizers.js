...
export async function searchProducts(supabase, term, limit, page = 1) {
  try {
    ...
    // Support multi-word token matching in addition to full phrase
    ...
    if (words.length > 1) {
      words.forEach(w => {
        filterClauses.push(`title.ilike.%${w}%`);
      });
    }
    ...
    const { data, error, count } = await supabase
      ...
      .select('id, title, description, category, slug, product_images, suggested_retail_price_paise, mrp_paise, admin_stock, fashion_product_categories(category_id)', { count: 'exact' })
      .or(filterStr)
      .eq('is_active', true)
      .is('deleted_at', null)
    ...
    const results = (data || []).map(row => {
      const isFashion = Boolean(row.fashion_product_categories && row.fashion_product_categories.length > 0);
      return {
        id: row.id,
        name: row.title,
        category: 'products',
        ...
        url: isFashion ? '/shop/fashion/product/' + row.id : '/shop/product/' + row.slug,
