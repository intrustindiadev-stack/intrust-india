import 'server-only';

import { createAdminClient } from '@/lib/supabaseServer';
import { normalisePhone } from '@/lib/omniflow';
import { sendWhatsAppWelcomeOnLink } from '@/lib/notifications/authWhatsapp';

const MERCHANT_ROLES = ['merchant', 'admin', 'super_admin'];

/**
 * Ensure a WhatsApp channel binding exists for the given user.
 *
 * - Looks up the user's role and phone from user_profiles.
 * - For merchant roles, prefers merchants.business_phone over profile phone.
 * - Upserts user_channel_bindings with whatsapp_opt_in = true.
 * - For merchant roles, also upserts merchant_notification_settings with all
 *   alert flags enabled (mirrors verify-otp/route.js lines 143–153).
 *
 * This function NEVER throws — all errors are caught and logged so it is safe
 * to call as a fire-and-forget side-effect or with await in a try/catch block.
 *
 * @param {{ userId: string }} options
 * @returns {Promise<{ linked: boolean, phone: string|null, audience: string|null, isNewLink: boolean }>}
 */
export async function ensureWhatsAppBinding({ userId }) {
  let audience = null;
  try {
    const admin = createAdminClient();

    // 1. Fetch role and phone from user_profiles
    const { data: profileRow, error: profileErr } = await admin
      .from('user_profiles')
      .select('role, phone')
      .eq('id', userId)
      .single();

    if (profileErr || !profileRow) {
      console.warn('[ensureWhatsAppBinding] Could not fetch user_profiles for user:', userId, profileErr?.message);
      return { linked: false, phone: null, audience: null, isNewLink: false };
    }

    // 2. Determine audience
    audience = MERCHANT_ROLES.includes(profileRow.role) ? 'merchant' : 'customer';

    // 3. Determine raw phone
    let rawPhone = profileRow.phone;
    if (audience === 'merchant') {
      const { data: merchantRow } = await admin
        .from('merchants')
        .select('id, business_phone')
        .eq('user_id', userId)
        .maybeSingle();

      // Prefer business_phone; fall back to profile phone
      rawPhone = merchantRow?.business_phone || profileRow.phone;
    }

    // 4. No phone — nothing to bind
    if (!rawPhone) {
      return { linked: false, phone: null, audience, isNewLink: false };
    }

    // 5. Normalise to E.164
    const normalisedPhone = normalisePhone(rawPhone);

    // 5.5 Check if the binding already exists for this (user_id, audience)
    let isNewLink = false;
    try {
      const { data: existingRow, error: selectErr } = await admin
        .from('user_channel_bindings')
        .select('phone')
        .eq('user_id', userId)
        .eq('audience', audience)
        .maybeSingle();

      if (selectErr) {
        console.warn('[ensureWhatsAppBinding] Select existing binding failed (non-fatal):', selectErr.message);
      }
      isNewLink = !existingRow;
    } catch (checkErr) {
      console.warn('[ensureWhatsAppBinding] Error checking existing binding (non-fatal):', checkErr.message);
      isNewLink = true;
    }

    // 6. Upsert user_channel_bindings
    //
    // Strategy: upsert on (user_id, audience) — handles the common case where
    // the user already has a binding and just needs their phone updated.
    //
    // Edge case: if another account already holds (phone, audience), the upsert
    // hits the unique constraint and PostgREST returns a bindErr. In that case
    // we forcibly reassign the conflicting row to this user — the current login
    // takes ownership of the binding. This prevents the silent skip that caused
    // merchants to receive zero WhatsApp notifications (Urban Mart incident).
    const now = new Date().toISOString();
    const { error: bindErr } = await admin
      .from('user_channel_bindings')
      .upsert(
        {
          user_id: userId,
          phone: normalisedPhone,
          audience,
          whatsapp_opt_in: true,
          linked_at: now,
          updated_at: now,
        },
        { onConflict: 'user_id,audience' }
      );

    if (bindErr) {
      const isPhoneConflict = bindErr.message?.includes('user_channel_bindings_phone_audience_key');
      if (isPhoneConflict) {
        // Another account owns this (phone, audience) — reassign it to the correct user.
        console.warn(
          `[ensureWhatsAppBinding] Phone ${normalisedPhone} already claimed by a different user` +
          ` under audience='${audience}'. Reassigning to correct user ${userId}.`
        );
        const { error: reassignErr } = await admin
          .from('user_channel_bindings')
          .update({ user_id: userId, whatsapp_opt_in: true, updated_at: now })
          .eq('phone', normalisedPhone)
          .eq('audience', audience);

        if (reassignErr) {
          console.warn('[ensureWhatsAppBinding] Binding reassign also failed (non-fatal):', reassignErr.message);
          return { linked: false, phone: normalisedPhone, audience, isNewLink: false };
        }
        console.log(`[ensureWhatsAppBinding] Binding successfully reassigned to user ${userId}.`);
        // isNewLink stays as determined above — continue normally
      } else {
        console.warn('[ensureWhatsAppBinding] Binding upsert failed (non-fatal):', bindErr.message);
        return { linked: false, phone: normalisedPhone, audience, isNewLink: false };
      }
    }

    // After a successful upsert, if isNewLink, call sendWhatsAppWelcomeOnLink
    if (isNewLink) {
      try {
        await sendWhatsAppWelcomeOnLink({ userId, audience, phone: normalisedPhone });
      } catch (welcomeErr) {
        console.error('[ensureWhatsAppBinding] Welcome send flow failed (non-fatal):', welcomeErr.message);
      }
    }

    // 7. For merchant audience, upsert merchant_notification_settings
    if (audience === 'merchant') {
      try {
        const { data: merchantData } = await admin
          .from('merchants')
          .select('id')
          .eq('user_id', userId)
          .maybeSingle();

        if (merchantData?.id) {
          await admin
            .from('merchant_notification_settings')
            .upsert(
              {
                merchant_id: merchantData.id,
                whatsapp_order_alerts: true,
                whatsapp_payout_alerts: true,
                whatsapp_store_credit_alerts: true,
                whatsapp_kyc_alerts: true,
                whatsapp_subscription_alerts: true,
                whatsapp_product_alerts: true,
                whatsapp_marketing: false,
                updated_at: now,
              },
              { onConflict: 'merchant_id' }
            );
        }
      } catch (settingsErr) {
        console.warn('[ensureWhatsAppBinding] merchant_notification_settings upsert failed (non-fatal):', settingsErr.message);
      }
    }

    // 8. Return success
    return { linked: true, phone: normalisedPhone, audience, isNewLink };
  } catch (err) {
    console.error('[ensureWhatsAppBinding] Unexpected error (non-fatal):', err.message);
    return { linked: false, phone: null, audience, isNewLink: false };
  }
}
