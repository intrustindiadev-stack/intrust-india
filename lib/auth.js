...
// and the legacy UUID format (phone-<uuid>@intrust.internal), as well as legacy @intrust-phone.com.
export const PSEUDO_EMAIL_RE =
    /^p\d+@phone\.intrust\.internal$|^phone-[0-9a-f-]{36}@intrust\.internal$|@intrust-phone\.com$/i;
...
export function isPseudoEmail(email) {
    ...
    return /@(phone\.)?intrust\.internal$|@intrust-phone\.com$/i.test(email);
}
 ...
 */
export async function redirectByRole(user, role, isSuspended, redirectUrl = null) {
    ...
    // If a safe internal redirect URL was provided, use it instead of role-based routing.
    ...
    } else if (role?.startsWith('relationship_') || role === 'relationship_exec' || role === 'relationship_manager') {
