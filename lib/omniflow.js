...
export const OTP_TEMPLATE = {
   ...
   * @returns {Array} Omniflow components array with button parameters.
   */
  // NOTE: The approved intrust_otp_verification template has BOTH:
  //   - body: {{1}} = the OTP code
  //   - button index 0: URL suffix = the OTP code
  // Confirmed by Meta API: expected: {body:1, button_index_params:[1]}
  // Both components MUST be sent together.
  buildComponents: (otp) => ([
    {
      type: 'body',
      parameters: [
...
/**
 * GM_QUOTE_TEMPLATE
 * Daily Good Morning broadcast with an admin-scheduled inspirational quote.
 * Replaces the alternating GM_GREET / GM_TIP pattern for the morning cron.
 * No finance or product language — fully neutral, suitable for all users.
 *
 * ─── OMNIFLOW TEMPLATE SPEC ─────────────────────────────────────────────────
 *   Template Name : intrust_gm_quote_v1
 *   Category      : Marketing
 *   Language      : en_US
 *   Header        : (none)
 *   Body          :
 ...
 *   Variables     :
 *     {{1}} = First name of the user (e.g. "Rahul")
 *     {{2}} = Daily quote text, admin-scheduled via daily_quotes table
 * ────────────────────────────────────────────────────────────────────────────
 *
 * @param {string} firstName - User's first name
 * @param {string} quoteText - Today's quote from the daily_quotes table
 * @returns {Array} Omniflow components array
 */
export const GM_QUOTE_TEMPLATE = {
  name: 'intrust_gm_quote_v1',
  language: 'en_US',
  buildComponents: (firstName, quoteText) => ([
    {
      type: 'body',
      parameters: [
        { type: 'text', text: String(firstName) },
        { type: 'text', text: String(quoteText) },
      ],
    },
  ]),
};
...
/**
 * Customer Utility Templates
 */
export const ORDER_STATUS_TEMPLATE = {
  name: 'intrust_order_status_v1',
  language: 'en_US',
  category: 'utility',
  buildComponents: (orderId, newStatus) => [
    {
      type: 'body',
      parameters: [
...
export const WALLET_LOW_BALANCE_TEMPLATE = {
  name: 'intrust_wallet_low_balance_v1',
  language: 'en_US',
  category: 'utility',
  buildComponents: (balance) => [
    {
      type: 'body',
      parameters: [
...
export const KYC_REMINDER_TEMPLATE = {
  name: 'intrust_kyc_reminder_v1',
  language: 'en_US',
  category: 'utility',
  buildComponents: (firstName, daysPending) => [
    {
      type: 'body',
      parameters: [
        { type: 'text', text: String(firstName) },
...
export const UDHARI_DUE_REMINDER_TEMPLATE = {
  name: 'intrust_udhari_due_reminder_v1',
  language: 'en_US',
  category: 'utility',
  buildComponents: (merchantName, amount, dueDate, status) => [
    {
      type: 'body',
      parameters: [
        { type: 'text', text: String(merchantName) },
        { type: 'text', text: String(amount) },
...
/**
 * Merchant Utility Templates
 */
export const MERCHANT_SUBSCRIPTION_EXPIRING_TEMPLATE = {
  name: 'intrust_merchant_subscription_expiring_v1',
  language: 'en_US',
  category: 'utility',
  buildComponents: (businessName, expiryDate) => [
    {
      type: 'body',
      parameters: [
...
export const MERCHANT_PAYOUT_FAILED_TEMPLATE = {
  name: 'intrust_merchant_payout_failed_v1',
  language: 'en_US',
  category: 'utility',
  buildComponents: (amount, reason) => [
    {
      type: 'body',
      parameters: [
...
export const INVESTMENT_MATURITY_TEMPLATE = {
  name: 'intrust_merchant_investment_maturity_v1',
  language: 'en_US',
  category: 'utility',
  buildComponents: (investmentType, amount, maturityDate) => [
    {
      type: 'body',
      parameters: [
        { type: 'text', text: investmentType },
        { type: 'text', text: amount },
...
/**
 * GE_GREET_TEMPLATE
 * Evening engagement message sent to users — personalised good evening greeting
 * with a soft CTA to review their day's finances in the InTrust wallet.
 *
 * ─── OMNIFLOW TEMPLATE SPEC ─────────────────────────────────────────────────
 *   Template Name : intrust_ge_greet_v1
 *   Category      : Marketing
 *   Language      : English (U.S.) — en_US
 *   Header        : (none)
 *   Body          :
 ...
 * @returns {Array} Omniflow components array
 */
export const GE_GREET_TEMPLATE = {
  name: 'intrust_ge_greet_v1',
  language: 'en_US',
  buildComponents: (firstName) => ([
    {
      type: 'body',
 ...
 * Sent when a customer's reward point balance crosses a configured milestone
 * (100, 500, 1000, 2500 or 5000 points). Fired from rewardNotifications.js.
 *
 * ─── OMNIFLOW TEMPLATE SPEC ─────────────────────────────────────────────────
 *   Template Name : intrust_reward_milestone_v1
 *   Category      : Marketing
 *   Language      : en_US
 *   Body          :
 ...
 *   Buttons       : [Quick Reply] View My Rewards, [Quick Reply] Redeem Points
 *   Variables     :
 *     {{1}} = Points just earned (e.g. "100")
 *     {{2}} = New total balance (e.g. "500")
 * ────────────────────────────────────────────────────────────────────────────
 */
export const REWARD_MILESTONE_TEMPLATE = {
  name: 'intrust_reward_milestone_v1',
  language: 'en_US',
  buildComponents: (pointsEarned, totalBalance) => ([
    {
      type: 'body',
      parameters: [
        { type: 'text', text: String(pointsEarned) },
        { type: 'text', text: String(totalBalance) },
      ],
    },
  ]),
};
 ...
 * Campaign broadcast — invites users to share their referral code.
 *
 * ─── OMNIFLOW TEMPLATE SPEC ─────────────────────────────────────────────────
 *   Template Name : intrust_referral_invite_v1
 *   Category      : Marketing
 *   Language      : en_US
 *   Body          :
 ...
 *   Buttons       : [Quick Reply] Share My Code, [Quick Reply] View Rewards
 *   Variables     :
 *     {{1}} = User's referral code (e.g. "ABC123")
 *     {{2}} = Bonus points per referral (e.g. "50")
 * ────────────────────────────────────────────────────────────────────────────
 */
export const REFERRAL_INVITE_TEMPLATE = {
  name: 'intrust_referral_invite_v1',
  language: 'en_US',
  buildComponents: (referralCode, bonusPoints) => ([
    {
      type: 'body',
      parameters: [
        { type: 'text', text: String(referralCode) },
        { type: 'text', text: String(bonusPoints) },
      ],
    },
  ]),
};
 ...
 * Campaign broadcast — promotes a gift card discount offer.
 *
 * ─── OMNIFLOW TEMPLATE SPEC ─────────────────────────────────────────────────
 *   Template Name : intrust_giftcard_promo_v1
 *   Category      : Marketing
 *   Language      : en_US
 *   Body          :
 ...
 *   Buttons       : [Quick Reply] Browse Gift Cards, [Quick Reply] My Wallet
 *   Variables     :
 *     {{1}} = Discount percentage (e.g. "10")
 *     {{2}} = Promo details / validity (e.g. "Use code SAVE10. Valid till 31 Jul 2026.")
 * ────────────────────────────────────────────────────────────────────────────
 */
export const GIFTCARD_PROMO_TEMPLATE = {
  name: 'intrust_giftcard_promo_v1',
  language: 'en_US',
  buildComponents: (discountPct, promoDetails) => ([
    {
      type: 'body',
      parameters: [
        { type: 'text', text: String(discountPct) },
        { type: 'text', text: String(promoDetails) },
      ],
    },
  ]),
};
...
/**
 * WINBACK_TEMPLATE
 * Sent by the weekly win-back cron to customers inactive for 30+ days.
 *
 * ─── OMNIFLOW TEMPLATE SPEC ─────────────────────────────────────────────────
 *   Template Name : intrust_winback_v1
 *   Category      : Marketing
 *   Language      : en_US
 *   Body          :
 *     👋 We miss you, *{{1}}*!
 ...
 *   Buttons       : [Quick Reply] Open My Account, [Quick Reply] View Offers
 *   Variables     :
 *     {{1}} = First name (e.g. "Rahul")
 *     {{2}} = Current reward balance (e.g. "250")
 * ────────────────────────────────────────────────────────────────────────────
 */
export const WINBACK_TEMPLATE = {
  name: 'intrust_winback_v1',
  language: 'en_US',
  buildComponents: (firstName, rewardBalance) => ([
    {
      type: 'body',
      parameters: [
        { type: 'text', text: String(firstName) },
        { type: 'text', text: String(rewardBalance) },
      ],
    },
  ]),
};
 ...
 * Campaign broadcast — announces a new product feature or platform update.
 *
 * ─── OMNIFLOW TEMPLATE SPEC ─────────────────────────────────────────────────
 *   Template Name : intrust_feature_announce_v1
 *   Category      : Marketing
 *   Language      : en_US
 *   Body          :
 ...
 *   Buttons       : [Quick Reply] Learn More, [Quick Reply] Open App
 *   Variables     :
 *     {{1}} = Feature name (e.g. "Lock-In Savings Plan")
 *     {{2}} = Short description (e.g. "Lock your savings for guaranteed returns — starting at just ₹500.")
 * ────────────────────────────────────────────────────────────────────────────
 */
export const FEATURE_ANNOUNCE_TEMPLATE = {
  name: 'intrust_feature_announce_v1',
  language: 'en_US',
  buildComponents: (featureName, description) => ([
    {
      type: 'body',
      parameters: [
        { type: 'text', text: String(featureName) },
        { type: 'text', text: String(description) },
      ],
    },
  ]),
};
 ...
 * Campaign broadcast — seasonal / festival greeting personalised to the user.
 *
 * ─── OMNIFLOW TEMPLATE SPEC ─────────────────────────────────────────────────
 *   Template Name : intrust_festival_greeting_v1
 *   Category      : Marketing
 *   Language      : en_US
 *   Body          :
 *     🎉 Happy *{{2}}*, *{{1}}*!
 ...
 *   Buttons       : [Quick Reply] View My Wallet, [Quick Reply] Explore Offers
 *   Variables     :
 *     {{1}} = First name (e.g. "Rahul")
 *     {{2}} = Festival name (e.g. "Diwali", "Eid", "New Year")
 * ────────────────────────────────────────────────────────────────────────────
 */
export const FESTIVAL_GREETING_TEMPLATE = {
  name: 'intrust_festival_greeting_v1',
  language: 'en_US',
  buildComponents: (firstName, festivalName) => ([
    {
      type: 'body',
      parameters: [
        { type: 'text', text: String(firstName) },
        { type: 'text', text: String(festivalName) },
      ],
    },
  ]),
};
 ...
 *
 * ─── OMNIFLOW TEMPLATE SPEC ─────────────────────────────────────────────────
 *   Template Name : intrust_merchant_payout_status_v2
 *   Category      : Utility
 *   Language      : English
 *   Header        : (none)
 *   Body          :
 ...
 *     {{1}} = Amount
 *     {{2}} = Status
 *     {{3}} = Reference/Note (e.g. "Automated Bank Settlement (NEFT / IMPS)" or "UTR / Ref: INT8392014")
 * ────────────────────────────────────────────────────────────────────────────
 *
 * @param {string} amountRs - Amount
...
export const MERCHANT_PAYOUT_STATUS_TEMPLATE = {
  name: 'intrust_merchant_payout_status_v2',
  language: 'en_US',
 ...
 * Sent on money events (wallet credits/debits).
 *
 * ─── OMNIFLOW TEMPLATE SPEC ─────────────────────────────────────────────────
 *   Template Name : intrust_merchant_transaction_alert_v1
 *   Category      : Utility
 *   Language      : English
 *   Header        : (none)
 *   Body          :
 ...
 * @param {string} direction - Credited or Debited
 * @param {string} newBalance - New balance
 * @param {string} reference - Source or reference
 * @returns {Array} Omniflow components array
 */
export const MERCHANT_TRANSACTION_ALERT_TEMPLATE = {
  name: 'intrust_merchant_transaction_alert_v1',
  language: 'en_US',
  buildComponents: (amount, direction, newBalance, reference) => ([
    {
      type: 'body',
      parameters: [
        { type: 'text', text: String(amount) },
        { type: 'text', text: String(direction) },
 ...
 * Sent to deliver customer invoice notifications and payment link.
 *
 * ─── OMNIFLOW TEMPLATE SPEC ─────────────────────────────────────────────────
 *   Template Name : intrust_invoice_notification_v1 (or WHATSAPP_INVOICE_TEMPLATE_NAME)
 *   Category      : Utility
 *   Language      : en_US (or WHATSAPP_INVOICE_TEMPLATE_LANG)
 *   Header        : InTrust India
 *   Body          :
 *     Hello {{1}},
 *
 *     Your invoice {{2}} for ₹{{3}} is ready.
 *
 *     Please use the button below to securely view your invoice and complete payment.
 *   CTA Button    : [URL Button] View & Pay Invoice
 *                   Target URL: https://www.intrustindia.com/pay/invoice/{{1}}
 *   Variables     :
 *     {{1}} = Customer name (e.g. "Rahul Sharma")
 *     {{2}} = Invoice number (e.g. "INV-2026-001")
 *     {{3}} = Amount (e.g. "1,180.00")
 *     Button 0 parameter = public_payment_token
 * ────────────────────────────────────────────────────────────────────────────
 */
export const INVOICE_NOTIFICATION_TEMPLATE = {
  name: process.env.WHATSAPP_INVOICE_TEMPLATE_NAME || 'intrust_invoice_notification_v1',
  language: process.env.WHATSAPP_INVOICE_TEMPLATE_LANG || 'en_US',
  category: 'utility',
  /**
   * @param {string} customerName
   * @param {string} invoiceNumber
   * @param {string} amount
   * @param {string} publicPaymentToken
   * @returns {Array} Omniflow components array
   */
  buildComponents: (customerName, invoiceNumber, amount, publicPaymentToken) => {
    const components = [
      {
        type: 'body',
        parameters: [
          { type: 'text', text: String(customerName || 'Valued Customer') },
    ...
    if (publicPaymentToken) {
      components.push({
        ...
        index: '0',
        parameters: [
          { type: 'text', text: String(publicPaymentToken) },
        ],
      });
    }
 ...
 * Sent to a merchant when an AI Order is assigned to their account.
 *
 * ─── OMNIFLOW TEMPLATE SPEC ─────────────────────────────────────────────────
 *   Template Name : intrust_ai_order_assigned (or WHATSAPP_AI_ORDER_TEMPLATE_NAME)
 *   Category      : Utility
 *   Language      : en_US (or WHATSAPP_AI_ORDER_TEMPLATE_LANG)
 *   Body          :
 *     Hello {{1}},
 *
 *     A new AI Order has been assigned to you.
 *
 *     Order: {{2}}
 *     Wholesale Amount: ₹{{3}}
 *     Expected Profit: ₹{{4}}
 *
 *     Please open your AI Orders panel to review and accept the order.
 *   CTA Button    : [URL Button] View AI Order
 *                   Target URL: https://www.intrustindia.com/merchant/ai-orders/{{1}}
 *   Variables     :
 *     {{1}} = Merchant Name (e.g. "Rahul Sharma" or business name)
 *     {{2}} = Order Code (e.g. "AI-1028")
 *     {{3}} = Wholesale Amount in ₹ (e.g. "50,000")
 *     {{4}} = Expected Profit in ₹ (e.g. "5,000")
 *     Button 0 parameter = orderId (optional dynamic route suffix)
 * ────────────────────────────────────────────────────────────────────────────
 */
export const AI_ORDER_ASSIGNED_TEMPLATE = {
  name: process.env.WHATSAPP_AI_ORDER_TEMPLATE_NAME || 'intrust_ai_order_assigned',
  language: process.env.WHATSAPP_AI_ORDER_TEMPLATE_LANG || 'en_US',
  category: 'utility',
  /**
   * @param {string} merchantName
   * @param {string} orderCode
   * @param {string} wholesaleAmount
   * @param {string} expectedProfit
   * @param {string} [orderId]
   * @returns {Array} Omniflow components array
   */
  buildComponents: (merchantName, orderCode, wholesaleAmount, expectedProfit, orderId) => {
    const components = [
      {
        type: 'body',
        parameters: [
          { type: 'text', text: String(merchantName || 'Merchant') },
          { type: 'text', text: String(orderCode) },
          { type: 'text', text: String(wholesaleAmount) },
          { type: 'text', text: String(expectedProfit) },
        ],
      },
    ];
...
export async function getAllTemplateStatuses() {
  assertConfig();
  try {
    const res = await fetch(`${BASE_URL}/api/wpbox/getTemplates?token=${TOKEN}`, {
      method: 'GET',
    });
    if (!res.ok) {
      const text = await res.text();
    ...
    const tracked = [
      ...
      REWARD_MILESTONE_TEMPLATE.name,
      REFERRAL_INVITE_TEMPLATE.name,
      GIFTCARD_PROMO_TEMPLATE.name,
      WINBACK_TEMPLATE.name,
      FEATURE_ANNOUNCE_TEMPLATE.name,
      FESTIVAL_GREETING_TEMPLATE.name,
      ORDER_STATUS_TEMPLATE.name,
      WALLET_LOW_BALANCE_TEMPLATE.name,
      KYC_REMINDER_TEMPLATE.name,
      UDHARI_DUE_REMINDER_TEMPLATE.name,
      MERCHANT_SUBSCRIPTION_EXPIRING_TEMPLATE.name,
      MERCHANT_PAYOUT_FAILED_TEMPLATE.name,
      INVESTMENT_MATURITY_TEMPLATE.name,
      MERCHANT_TRANSACTION_ALERT_TEMPLATE.name,
      INVOICE_NOTIFICATION_TEMPLATE.name,
