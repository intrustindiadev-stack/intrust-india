...
export function isOrderInPeriod(order, periodKey, refDate = new Date()) {
    ...
    // For packed/shipped/delivered orders, the sale date is when it was marked packed
    // For pending/cancelled orders, the date is when the order was placed (created_at)
    const isPacked = ['packed', 'shipped', 'delivered'].includes((order.delivery_status || '').toLowerCase());
    const relevantDate = isPacked && order.packed_at ? order.packed_at : order.created_at;
    if (!relevantDate) return true;

    const orderTime = new Date(relevantDate).getTime();
...
/**
 * Checks if an order counts as a confirmed sale.
 * A sale only counts when the product is marked as packed (packed, shipped, delivered).
 * Pending orders are unfulfilled requests awaiting packing and are not yet sales.
 * @param {object} order 
 * @returns {boolean}
 */
export function isPackedOrder(order) {
    if (!isValidOrder(order)) return false;
    const deliveryStatus = (order.delivery_status || '').toLowerCase();
    return ['packed', 'shipped', 'delivered'].includes(deliveryStatus);
}
...
export function calculateMerchantOrderKPIs(orders = [], periodKey = 'all', refDate = new Date()) {
    ...
    // Total Sales only counts orders marked as packed (packed, shipped, delivered)
    const boundary = getPeriodBoundary(periodKey, refDate);
    const packedOrders = periodOrders.filter(o => {
        if (!isPackedOrder(o)) return false;
        if (!boundary) return true;
        const packedTime = o.packed_at ? new Date(o.packed_at).getTime() : new Date(o.created_at).getTime();
        return packedTime >= boundary.getTime();
    });

    // 1. Total Sales: Sum of customer order values for packed/fulfilled orders
    const totalSalesPaise = packedOrders.reduce((sum, o) => sum + (Number(o.total_amount_paise) || 0), 0);
    const packedOrdersCount = packedOrders.length;

    // 2. Orders: Number of sales orders (packed/fulfilled)
    const validOrdersCount = packedOrdersCount;
    ...
    return {
        ...
        totalSalesSubtext: `${packedOrdersCount} packed order${packedOrdersCount === 1 ? '' : 's'}`,

        validOrdersCount,
        packedOrdersCount,
        allValidOrdersCount: validOrders.length,
        totalOrdersCount,
        ordersFormatted: packedOrdersCount.toString(),
        ordersSubtext: periodKey === 'all' ? 'All packed orders' : `In ${periodLabel.toLowerCase()}`,
