 ...
 * Keys are Base64 encoded strings from the Sabpaisa panel.
 */
function getKeys(authKeyOverride, authIVOverride) {
    ...
    let aesKey = Buffer.from(authKey, 'base64');
    let hmacKey = Buffer.from(authIV, 'base64');

    // 🔴 Fallback logic: If keys are 16-char raw strings, 
    // base64 decoding results in 12 bytes which is invalid.
    // If we detect this, or if the user didn't use base64, fallback to UTF-8.
    if (aesKey.length !== 16 && aesKey.length !== 32) {
        console.warn(`[Sabpaisa] authKey Base64 decode resulted in ${aesKey.length} bytes. Falling back to UTF-8.`);
        aesKey = Buffer.from(authKey, 'utf8');
    }

    if (hmacKey.length < 16) {
        console.warn(`[Sabpaisa] authIV Base64 decode resulted in ${hmacKey.length} bytes. Falling back to UTF-8.`);
        hmacKey = Buffer.from(authIV, 'utf8');
    }

    console.log(`[Sabpaisa] Real Key Lengths: AES=${aesKey.length} bytes, HMAC=${hmacKey.length} bytes`);
