...
export async function fulfillTransaction(supabaseAdmin, txn, internalStatus, gatewayPayload) {
    ...
    if (!fulfillmentFailed && txn.udf1 === 'MERCHANT_LOCKIN') {
        try {
            ...
            // Cheap sequential-retry short-circuit: log and bail early if this txn was
            // already applied. Concurrent races are handled below by the DB unique index.
            const { data: existingLockin } = await supabaseAdmin
                .from('merchant_lockin_balances')
                .select('id')
                .eq('merchant_id', merchantCheck.id)
            ...
            if (existingLockin) {
                ...
                // The DB partial unique index on gateway_txn_id is the definitive race guard. 
                // We use standard insert; if two concurrent requests arrive simultaneously,
                // the second insert hits the constraint, throws 23505, and is safely swallowed below.
                ...
                if (lockinErr) {
                    if (lockinErr.code === '23505' || lockinErr.message?.includes('23505')) {
                        console.warn(`[Fulfillment] MERCHANT_LOCKIN unique constraint hit for txn ${clientTxnId} — concurrent duplicate suppressed.`);
                        return { fulfillmentFailed, internalStatus, transMsg, fulfillmentComplete: !fulfillmentFailed };
                    }
                    throw lockinErr;
                }
    ...
    if (!fulfillmentFailed && txn.udf1 === 'MERCHANT_AIGROW') {
        try {
            ...
            if (existingInvestment) {
                ...
                if (aiGrowErr) {
                    if (aiGrowErr.code === '23505' || aiGrowErr.message?.includes('23505')) {
