...
export async function fulfillTransaction(supabaseAdmin, txn, internalStatus, gatewayPayload) {
    ...
    if (!fulfillmentFailed && txn.udf1 === 'MERCHANT_LOCKIN') {
        try {
            ...
            const { data: merchantCheck, error: ownerErr } = await supabaseAdmin.from('merchants').select('id').eq('user_id', txn.user_id).single();
            if (ownerErr || !merchantCheck) {
                throw new Error('Merchant ownership verification failed.');
            }

            const { error: lockinErr } = await supabaseAdmin.from('merchant_lockin_balances').insert({
    ...
    if (!fulfillmentFailed && txn.udf1 === 'MERCHANT_AIGROW') {
        try {
            ...
            const { error: aiGrowErr } = await supabaseAdmin.from('merchant_investments').insert({
                merchant_id: merchantCheck.id,
                amount_paise: amountPaise,
