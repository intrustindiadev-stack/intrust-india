...
import { MERCHANT_SUBSCRIPTION_PLANS, GOLD_SUBSCRIPTION_PLANS, MERCHANT_INVESTMENT_TERMS } from '@/lib/constants';
 ...
 * @returns {{ fulfillmentFailed: boolean, internalStatus: string, transMsg: string, fulfillmentComplete: boolean }}
 */
export async function fulfillTransaction(supabaseAdmin, txn, internalStatus, gatewayPayload) {
    ...
    let fulfillmentComplete = false; // set true only when all steps finish without error

    if (!txn || internalStatus !== 'gateway_success') {
        return { fulfillmentFailed, internalStatus, transMsg, fulfillmentComplete };
    }

    // ─── WALLET_TOPUP ────────────────────────────────────────────────────────
    if (txn.udf1 === 'WALLET_TOPUP') {
        try {
            // Idempotency: check customer_wallet_transactions for an existing row
            // keyed on reference_id = clientTxnId before crediting. This prevents
            // a second credit if the route retries after fulfilled_at stamping fails.
            const { data: existingWalletTx } = await supabaseAdmin
                .from('customer_wallet_transactions')
                .select('id')
                .eq('reference_id', clientTxnId)
                .eq('reference_type', 'TOPUP')
                .maybeSingle();

            if (existingWalletTx) {
                console.log(`[Fulfillment] WALLET_TOPUP already credited for txn ${clientTxnId} — skipping.`);
            } else {
                const creditResult = await CustomerWalletService.creditWallet(
                    txn.user_id,
                    amount,
                    'TOPUP',
                ...
                if (creditResult && creditResult.duplicate) {
                    console.warn(`[Fulfillment] WALLET_TOPUP unique constraint hit for txn ${clientTxnId} — concurrent duplicate suppressed.`);
                } else {
            ...
            try {
                ...
                if (affiliateCode) {
                    ...
                    const { data: refResult, error: refError } = await supabaseAdmin.rpc('process_marketing_referral_reward', {
                        ...
                        p_converted_user_id: txn.user_id,
                        p_product_id: orderItems?.product_id || null,
                        p_reference_id: groupId
                    });

                    if (refError || (refResult && !refResult.success)) {
                        console.warn('[Fulfillment] Marketing referral reward skipped/failed:', refError?.message || refResult?.message);
                    }
                }
            } catch (affiliateErr) {
                console.error('[Fulfillment] Affiliate referral reward error:', affiliateErr?.message);
            }
    ...
    // ─── DAILY_CHALLENGE_SPONSORSHIP ──────────────────────────────────────────
    if (!fulfillmentFailed && txn.udf1 === 'DAILY_CHALLENGE_SPONSORSHIP') {
        try {
            const sponsorDate = txn.udf2;
            const merchantId = txn.udf3;
            const amountPaise = Math.round(parseFloat(amount) * 100);

            // Check if ledger row already inserted (e.g. by API route)
            const { data: existingLedger } = await supabaseAdmin
                .from('merchant_transactions')
                .select('id')
                .eq('merchant_id', merchantId)
                .eq('transaction_type', 'sponsorship')
                .contains('metadata', { client_txn_id: clientTxnId })
                .maybeSingle();

            if (!existingLedger && merchantId) {
                const { data: merchant } = await supabaseAdmin
                    .from('merchants')
                    .select('id, user_id, wallet_balance_paise')
                    .eq('id', merchantId)
                    .single();
                ...
                if (merchant) {
                    await supabaseAdmin.from('merchant_transactions').insert({
                        ...
                        amount_paise: amountPaise,
                        balance_after_paise: merchant.wallet_balance_paise,
                    ...
                    await supabaseAdmin.from('notifications').insert({
                        user_id: merchant.user_id,
                        title: 'Daily Challenge Sponsorship Confirmed! 🏆',
                        body: `Your daily challenge sponsorship for ${sponsorDate} has been confirmed.`,
                        type: 'success',
                        reference_id: clientTxnId,
                        reference_type: 'daily_challenge_sponsorship'
                    });
                }
            }
        } catch (sponErr) {
            console.error('[Fulfillment] DAILY_CHALLENGE_SPONSORSHIP error:', sponErr.message);
        }
    }
    ...
    if (!fulfillmentFailed && txn.udf1 === 'GOLD_SUBSCRIPTION') {
        try {
            ...
            const { data: profile } = await supabaseAdmin.from('user_profiles').select('is_gold_verified, subscription_expiry, gateway_txn_id').eq('id', txn.user_id).single();

            // Idempotency: if this exact clientTxnId already applied the subscription, skip.
            if (profile?.gateway_txn_id === clientTxnId) {
                console.log(`[Fulfillment] GOLD_SUBSCRIPTION already applied for txn ${clientTxnId} — skipping.`);
            } else {
                ...
                // Write gateway_txn_id alongside the subscription update as a durable idempotency marker.
                await supabaseAdmin.from('user_profiles').update({
                    is_gold_verified: true,
                    subscription_expiry: newExpiryDate.toISOString(),
                    gateway_txn_id: clientTxnId
                }).eq('id', txn.user_id);
    ...
    if (!fulfillmentFailed && txn.udf1 === 'MERCHANT_SUBSCRIPTION') {
        try {
            ...
            const { data: merchantCheck } = await supabaseAdmin.from('merchants').select('subscription_status, subscription_expires_at, last_sub_gateway_txn_id').eq('id', merchantId).single();

            // Idempotency: if this exact clientTxnId already activated/renewed the subscription, skip.
            if (merchantCheck?.last_sub_gateway_txn_id === clientTxnId) {
                console.log(`[Fulfillment] MERCHANT_SUBSCRIPTION already applied for txn ${clientTxnId} — skipping.`);
            } else {
                ...
                // Write last_sub_gateway_txn_id alongside subscription update as a durable idempotency marker.
                const { error: merchUpdateErr } = await supabaseAdmin.from('merchants').update({
                    subscription_status: 'active',
                    subscription_expires_at: newExpiry,
                    last_sub_gateway_txn_id: clientTxnId
                }).eq('id', merchantId);
    ...
    // ─── MERCHANT_LOCKIN ─────────────────────────────────────────────────────
    if (!fulfillmentFailed && txn.udf1 === 'MERCHANT_LOCKIN') {
        try {
            const amountPaise = Math.round(parseFloat(amount) * 100);

            const { data: merchantCheck, error: ownerErr } = await supabaseAdmin
                .from('merchants').select('id').eq('user_id', txn.user_id).single();
            if (ownerErr || !merchantCheck) {
                throw new Error('Merchant ownership verification failed.');
            }

            const { data: existingLockin } = await supabaseAdmin
                .from('merchant_lockin_balances')
                .select('id')
                .eq('gateway_txn_id', clientTxnId)
                .maybeSingle();

            if (existingLockin) {
                console.log(`[Fulfillment] MERCHANT_LOCKIN already applied for txn ${clientTxnId} — skipping.`);
            } else {
                ...
                const { error: lockinErr } = await supabaseAdmin
                    ...
                    .insert({
                        ...
                        status: 'active',
                        gateway_txn_id: clientTxnId,
                        notes: txn.udf2 || 'Lockin request'
                    });
                
                if (lockinErr) {
                    if (lockinErr.code === '23505') {
                        console.warn(`[Fulfillment] MERCHANT_LOCKIN unique constraint hit for txn ${clientTxnId} — concurrent duplicate suppressed.`);
                    } else {
                        throw lockinErr;
                    }
                } else {
                    await supabaseAdmin.from('notifications').insert({
                        ...
                        body: `Your Lockin portfolio has been funded with ₹${amount}.`,
                        type: 'success'
                    });
                }
            }
        } catch (err) {
            console.error('[Fulfillment] MERCHANT_LOCKIN failed:', err.message);
            fulfillmentFailed = true;
            internalStatus = 'failed';
            transMsg = 'Lockin funding failed. Manual verification required. Contact support.';
        }
    }

    // ─── MERCHANT_AIGROW ─────────────────────────────────────────────────────
    if (!fulfillmentFailed && txn.udf1 === 'MERCHANT_AIGROW') {
        try {
            const amountPaise = Math.round(parseFloat(amount) * 100);
            const description = txn.udf2 || 'AI Grow Request';
            ...
            if (existingInvestment) {
                console.log(`[Fulfillment] MERCHANT_AIGROW already applied for txn ${clientTxnId} — skipping.`);
            } else {
                ...
                if (aiGrowErr) {
                    if (aiGrowErr.code === '23505') {
                        console.warn(`[Fulfillment] MERCHANT_AIGROW unique constraint hit for txn ${clientTxnId} — concurrent duplicate suppressed.`);
                    } else {
                        throw aiGrowErr;
                    }
                } else {
                    // Keep the authoritative ai_grow_wallets ledger in sync:
                    // credit the vault for this confirmed gateway deposit.
                    // A ledger failure must fail fulfillment so the payment is
                    // flagged for manual verification rather than silently
                    // landing in rows without vault credit.
                    const ledger = await creditAiGrowVaultForDeposit({
                        merchantId: merchantCheck.id,
                        amountRupees: amountPaise / 100,
                        gatewayTxnId: clientTxnId,
                        source: 'sabpaisa_fulfillment',
                    });
                    ...
                    await supabaseAdmin.from('notifications').insert({
                        user_id: txn.user_id,
                        title: 'AI Grow Request Funded ✅',
                        body: `Your AI Grow request for ₹${amount} has been successfully paid and submitted.`,
                        type: 'success'
                    });
                }
            }
        } catch (err) {
            console.error('[Fulfillment] MERCHANT_AIGROW failed:', err.message);
            fulfillmentFailed = true;
            internalStatus = 'failed';
            transMsg = 'AI Grow funding failed. Manual verification required. Contact support.';
        }
    }

    // ─── AI_ORDER ────────────────────────────────────────────────────────────
    if (!fulfillmentFailed && txn.udf1 === 'AI_ORDER') {
        try {
            const aiOrderId = txn.udf2;

            const { data: aiOrder, error: orderErr } = await supabaseAdmin
                .from('ai_orders')
                .select('id, status')
                .eq('id', aiOrderId)
                .single();
            ...
            if (orderErr || !aiOrder) {
                throw new Error(`AI Order not found: ${aiOrderId}`);
            }

            if (aiOrder.status === 'ACCEPTED' || aiOrder.status === 'COMPLETED') {
                console.log(`[Fulfillment] AI_ORDER already accepted for txn ${clientTxnId} — skipping.`);
            } else if (aiOrder.status === 'PAYMENT_PENDING' || aiOrder.status === 'PENDING') {
                const { error: updateErr } = await supabaseAdmin
                    .from('ai_orders')
                    .update({
                        status: 'ACCEPTED',
                        payment_method: paymentMode || 'SabPaisa PG',
                ...
                await supabaseAdmin.from('notifications').insert({
                    user_id: txn.user_id,
                    title: 'AI Order Payment Received ✅',
                    body: `Your payment of ₹${amount} for the AI Order has been successfully received.`,
                    type: 'success'
                });
            } else {
                console.warn(`[Fulfillment] AI Order ${aiOrderId} is in unexpected status ${aiOrder.status}. Skipping.`);
            }
        } catch (err) {
            console.error('[Fulfillment] AI_ORDER failed:', err.message);
            fulfillmentFailed = true;
            internalStatus = 'failed';
            transMsg = 'AI Order fulfillment failed. Manual verification required. Contact support.';
        }
    }

    // ─── INVOICE_PAY ─────────────────────────────────────────────────────────
    if (!fulfillmentFailed && txn.udf1 === 'INVOICE_PAY') {
        try {
            const invoiceId = txn.udf2;
            const amountPaise = Math.round(parseFloat(amount) * 100);

            const { data: invoice, error: invErr } = await supabaseAdmin
                .from('invoices')
                .select('id, status, grand_total_paise, amount_paid_paise')
                .eq('id', invoiceId)
                .single();
            ...
            if (invoice.status === 'PAID') {
                console.warn(`[Fulfillment] DUPLICATE_PAYMENT detected: Invoice ${invoiceId} is already PAID. Skipping fulfillment.`);
                await supabaseAdmin.from('invoice_events').insert({
                    invoice_id: invoiceId,
                    event_type: 'DUPLICATE_PAYMENT',
                    description: `Duplicate payment of ₹${amount} received via SabPaisa for already PAID invoice. Manual refund or reconciliation required.`,
                    metadata: {
                        client_txn_id: clientTxnId,
                        sabpaisa_txn_id: gatewayPayload?.sabpaisaTxnId || txn.sabpaisa_txn_id || null,
                ...
                console.warn(`[Fulfillment] Payment received for ${invoice.status} invoice ${invoiceId}. Refusing to fulfill.`);
                await supabaseAdmin.from('invoice_events').insert({
                    invoice_id: invoiceId,
                    event_type: 'DUPLICATE_PAYMENT',
                    description: `Payment of ₹${amount} received for ${invoice.status} invoice. Refusing to mark as PAID. Manual refund required.`,
                    metadata: {
                        client_txn_id: clientTxnId,
                        sabpaisa_txn_id: gatewayPayload?.sabpaisaTxnId || txn.sabpaisa_txn_id || null,
                        amount: Number(amount),
                ...
                fulfillmentFailed = true;
                internalStatus = 'failed';
                transMsg = `Payment received for ${invoice.status} invoice. Manual refund required. Contact support.`;
            } else if (invoice.status === 'ISSUED' || invoice.status === 'PARTIALLY_PAID') {
                const totalPaidNow = invoice.amount_paid_paise + amountPaise;
                const newStatus = totalPaidNow >= invoice.grand_total_paise ? 'PAID' : 'PARTIALLY_PAID';

                const { data: updatedRows, error: updateErr } = await supabaseAdmin
                    .from('invoices')
                    .update({
                        status: newStatus,
                        amount_paid_paise: totalPaidNow,
                ...
                if (updatedRows && updatedRows.length > 0) {
                    console.log(`[Fulfillment] Invoice ${invoiceId} marked as ${newStatus} for txn ${clientTxnId}`);
                    
                    await supabaseAdmin.from('invoice_events').insert({
                        invoice_id: invoiceId,
                        event_type: newStatus === 'PAID' ? 'INVOICE_PAID' : 'PARTIALLY_PAID',
                        description: `Payment of ₹${amount} completed via SabPaisa (${paymentMode || 'Gateway'}). Invoice is now ${newStatus}.`,
                        metadata: {
                            client_txn_id: clientTxnId,
                            sabpaisa_txn_id: gatewayPayload?.sabpaisaTxnId || txn.sabpaisa_txn_id || null,
                            amount: Number(amount),
                    ...
                    // Trigger Payment Notification (non-blocking, idempotent)
                    try {
                        const notifType = newStatus === 'PAID' ? 'PAYMENT_SUCCESS' : 'PARTIAL_PAYMENT';
                        ...
                        console.error('[Fulfillment] Invoice payment notification error:', notifErr.message);
                    }
                } else {
                    console.warn(`[Fulfillment] Concurrency race: Invoice ${invoiceId} was updated by another process. Skipping.`);
                }
            } else {
                console.warn(`[Fulfillment] Unknown invoice status ${invoice.status} for invoice ${invoiceId}. Skipping.`);
            }
        } catch (err) {
            console.error('[Fulfillment] INVOICE_PAY failed:', err.message);
            fulfillmentFailed = true;
            internalStatus = 'failed';
            transMsg = 'Invoice payment fulfillment failed. Manual verification required. Contact support.';
        }
    }

    // ─── AI_ORDER ────────────────────────────────────────────────────────────
    if (txn.udf1 === 'AI_ORDER') {
        try {
            const orderId = txn.udf2;
            if (orderId) {
                const { error: orderErr } = await supabaseAdmin
                    .from('ai_orders')
                    .update({
                        status: 'ACCEPTED',
                        payment_method: paymentMode || 'SABPAISA',
                        payment_received_at: new Date().toISOString(),
                        sabpaisa_txn_id: gatewayPayload?.sabpaisaTxnId || txn.sabpaisa_txn_id || null,
                        updated_at: new Date().toISOString()
                    })
                ...
                if (orderErr) {
                    console.error('[Fulfillment] Failed to update AI order status:', orderErr);
                    throw orderErr;
                }
                console.log(`[Fulfillment] AI Order ${orderId} successfully marked as ACCEPTED`);
            }
        } catch (err) {
            console.error('[Fulfillment] AI_ORDER fulfillment failed:', err.message);
            fulfillmentFailed = true;
            internalStatus = 'failed';
            transMsg = 'AI order payment fulfillment failed. Please contact support.';
        }
    }

    fulfillmentComplete = !fulfillmentFailed;
    return { fulfillmentFailed, internalStatus, transMsg, fulfillmentComplete };
}
