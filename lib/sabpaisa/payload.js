...
export function buildEncryptedPayload(orderData) {
    // 🔴 KIT 2.0 APPROACH: RAW Query String + AES-256-GCM + HMAC-SHA-384
    // Modern SabPaisa systems use GCM for encryption and HMAC for integrity.

    // Keep the clientTxnId as-is — truncating or uppercasing it breaks callback matching.
    const txnId = (orderData.clientTxnId || '').trim();

    const payerEmail = (orderData.payerEmail || '').trim() || 'guest@sabpaisa.in';

    // 🟢 Strictly follow the SDK's field list and order (41 fields)
    const params = new URLSearchParams({
        ...
        channelId: 'W',
        // mcc and transDate are required by Kit 2.0
        mcc: '',
    ...
    // 🟢 Use the default string representation (uses '+' for spaces, matching the SDK)
