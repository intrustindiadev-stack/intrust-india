 ...
 * SabPaisa Node.js 2.0 SDK uses HEX encoding for both Webhook callbacks
 * and Status Inquiry API responses.
 *
 * @param {string} cipherText - Encoded encrypted string
 * @param {string} encoding - Default is 'hex'
 * @returns {string | null} Decrypted plaintext, or null on failure
 */
export function decrypt(cipherText, encoding = 'hex') {
    if (!cipherText) return null;

    try {
        const { aesKey, hmacKey } = getKeys();

        const fullMessage = Buffer.from(cipherText, encoding);
