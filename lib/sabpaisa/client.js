...
class SabpaisaClient {
    ...
    parseResponse(encryptedResponse) {
        try {
            // Webhook responses are Base64 encoded
            const decryptedString = decrypt(encryptedResponse, 'base64');
    ...
    async verifyTransaction(clientTxnId) {
        try {
            ...
            // 4. Decrypt the statusResponseData field (Status Inquiry API returns Hex)
            ...
            const decryptedString = decrypt(jsonResponse.statusResponseData, 'hex');
     ...
     * 
     * @param {object} refundData { sabpaisaTxnId, amount, clientTxnId }
     * @returns {object} Refund response from gateway
     */
    async initiateRefund(refundData) {
        try {
            const { sabpaisaTxnId, amount, clientTxnId } = refundData;
            
            // 1. Build the query string for refund encryption
            // Note: Sabpaisa refund API typically requires clientCode, sabpaisaTxnId, and amount
            const params = new URLSearchParams({
                clientCode: this.config.clientCode,
            ...
            const refundEncData = encrypt(queryStr);

            if (!refundEncData) {
                throw new Error('Failed to encrypt refund payload');
            }

            // 2. POST to the refund endpoint (placeholder URL as specific Sabpaisa refund endpoint 
            // varies by API version, typically /SabPaisa/refundApi)
            const refundUrl = this.config.baseUrl + '/SabPaisa/refundApi';

            console.log(`[Sabpaisa] Initiating refund for txn ${sabpaisaTxnId}, amount ${amount}`);

            // This is a placeholder for the actual fetch call once credentials/endpoints are fully verified
            // for the specific merchant account version.
            /*
            ...
            */
            ...
            return {
                status: 'REQUESTED',
                message: 'Refund orchestration initiated. Manual approval may be required in SabPaisa dashboard.',
                txnId: sabpaisaTxnId
            };

        } catch (error) {
            console.error('[Sabpaisa] Refund initiation error:', error);
            return {
                status: 'ERROR',
                message: error.message
            };
