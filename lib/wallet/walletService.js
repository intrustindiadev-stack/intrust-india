...
export const WalletService = {
    ...
    async debitWallet(userId, amount, referenceId, referenceType, description) {
        ...
        // 1. Get current merchant record (same source of truth as creditWallet)
        const { data: merchant, error: merchantFetchError } = await supabaseAdmin
            .from('merchants')
            .select('id, wallet_balance_paise')
            .eq('user_id', userId)
        ...
        // 2. Update merchant wallet balance BEFORE inserting transaction record
        //    If this fails, no transaction row is inserted (atomic debit)
        const { error: updateError } = await supabaseAdmin
            .from('merchants')
            .update({
                wallet_balance_paise: newBalancePaise,
        ...
        if (txError) {
            // Non-fatal: balance was already deducted, just log the history failure
            console.error('[WalletService] wallet_transactions insert failed:', txError);
        }
