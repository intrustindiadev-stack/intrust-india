...
import { notifyCustomerWalletLowBalance } from '@/lib/notifications/userWhatsapp';
...
export const CustomerWalletService = {
    ...
    async creditWallet(userId, amount, type, description, reference = {}, auditLog = null) {
        ...
        // Non-admin path: use the atomic Postgres function to prevent race conditions
        const { data, error } = await supabaseAdmin.rpc('atomic_customer_wallet_credit', {
            p_user_id: userId,
            p_amount_paise: amountPaise,
            p_type: type,
            p_description: description,
            p_reference_id: reference?.id || null,
        ...
        return {
            ...
            newBalance: data.balance_after_paise / 100,
            transaction: { id: data.transaction_id },
            auditLogId: null
        };
    },
    ...
    async debitWallet(userId, amount, description, reference = {}, auditLog = null) {
        ...
        if (auditLog) {
            ...
            if (!data.duplicate && (data.balance_after_paise || 0) < 5000) {
                notifyCustomerWalletLowBalance({ userId, balance: (data.balance_after_paise / 100).toFixed(2) })
                    .catch(e => console.error('[Wallet] Low balance notify error:', e));
            }
        ...
        if (balanceAfter < 5000) {
            notifyCustomerWalletLowBalance({ userId, balance: (balanceAfter / 100).toFixed(2) })
                .catch(e => console.error('[Wallet] Low balance notify error:', e));
        }
