      ...
      const merchantPromise = safeQuery(
        ...
        if (merchant) {
          ...
          const pendingFulfillmentsP = safeQuery(
            ...
            context.pendingFulfillmentsCount = (r?.data || []).filter(isPendingActionOrder).length;
          });
          ...
          const lockinP = safeQuery(
            adminClient.from('merchant_lockin_balances')
              .select('amount_paise, accumulated_interest_paise, status, start_date, end_date, interest_rate')
              .eq('merchant_id', merchant.id)
            ...
            if (r?.data) {
              ...
              context.lockinInterestRs = lockinRows.reduce((sum, row) => {
                let interestPaise = row.accumulated_interest_paise || 0;
                if (row.start_date) {
                  ...
                  const calcInterest = Math.round(row.amount_paise * (row.interest_rate / 100 / 365) * days);
                  interestPaise = Math.max(interestPaise, calcInterest);
                }
                return sum + interestPaise / 100;
              }, 0);
            }
          });
