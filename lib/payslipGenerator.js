...
export const downloadPayslip = async ({ employee, salary, lineItems, attendanceStats }) => {
    const doc = new jsPDF();
    ...
    // ── Attendance Summary ──
    if (attendanceStats) {
        y = sectionHeader(doc, 'ATTENDANCE SUMMARY', y, margin, contentWidth);
        doc.setFontSize(8.5);
        doc.setTextColor(SLATE.r, SLATE.g, SLATE.b);
        
        let tx = margin;
        
        doc.setFont('helvetica', 'normal'); doc.text('Present:', tx, y);
        doc.setFont('helvetica', 'bold'); doc.setTextColor(EMERALD.r, EMERALD.g, EMERALD.b); doc.text(`${attendanceStats.present || 0}`, tx + 18, y);
        
        tx += 45;
        doc.setFont('helvetica', 'normal'); doc.setTextColor(SLATE.r, SLATE.g, SLATE.b); doc.text('Absent:', tx, y);
        doc.setFont('helvetica', 'bold'); doc.setTextColor(ROSE.r, ROSE.g, ROSE.b); doc.text(`${attendanceStats.absent || 0}`, tx + 16, y);
        
        tx += 45;
        doc.setFont('helvetica', 'normal'); doc.setTextColor(SLATE.r, SLATE.g, SLATE.b); doc.text('Half Day:', tx, y);
        doc.setFont('helvetica', 'bold'); doc.setTextColor(NAVY.r, NAVY.g, NAVY.b); doc.text(`${attendanceStats.half_day || 0}`, tx + 18, y);

        tx += 45;
        doc.setFont('helvetica', 'normal'); doc.setTextColor(SLATE.r, SLATE.g, SLATE.b); doc.text('Late:', tx, y);
        doc.setFont('helvetica', 'bold'); doc.setTextColor(NAVY.r, NAVY.g, NAVY.b); doc.text(`${attendanceStats.late || 0}`, tx + 12, y);
