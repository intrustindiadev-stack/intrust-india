...
import { classifyMonthAttendanceAndLeaves } from '@/lib/hrm/payroll';
...
export const downloadPayslip = async ({ employee, salary, lineItems, attendanceStats: initialAttendanceStats }) => {
    ...
    if (!attendanceStats && salary && employee && employee.id) {
        try {
            ...
            const [attsRes, leavesRes, polRes, holsRes] = await Promise.all([
                ...
                supabase.from('leave_requests')
                    .select('from_date, to_date, status, leave_type, chargeable_days, policy:leave_policies(is_paid)')
                    .eq('employee_id', employee.id)
                    .eq('status', 'approved')
                    .lte('from_date', ed)
                    .gte('to_date', sd),
                supabase.from('organization_policy').select('weekend_days').limit(1).maybeSingle(),
                supabase.from('holidays').select('holiday_date').gte('holiday_date', sd).lte('holiday_date', ed).eq('is_optional', false)
            ]);
            ...
            const holidays = (holsRes.data || []).map(h => h.holiday_date);
    ...
    if (attendanceStats) {
        ...
        const paidL = attendanceStats.paid_leave || 0;
        const unpaidL = attendanceStats.unpaid_leave || 0;
        const wkDays = attendanceStats.workingDays || attendanceStats.expectedWorkingDays || totalCalendarDays;
        const wOffs = attendanceStats.weeklyOffs !== undefined ? attendanceStats.weeklyOffs : 4;
        const hols = attendanceStats.holidays !== undefined ? attendanceStats.holidays : 0;

        doc.setFontSize(8);
        const colW = contentWidth / 4;
        
        const drawAttItem = (lbl, val, xPos, yPos, valColor = NAVY) => {
            doc.setFont('helvetica', 'normal');
            doc.setTextColor(SLATE.r, SLATE.g, SLATE.b);
            doc.text(lbl, xPos, yPos);
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(valColor.r, valColor.g, valColor.b);
            doc.text(String(val), xPos + 26, yPos);
        };

        // Row 1
        drawAttItem('Calendar Days:', totalCalendarDays, margin, y, NAVY);
        drawAttItem('Working Days:', wkDays, margin + colW, y, NAVY);
        drawAttItem('Present Days:', pres, margin + colW * 2, y, EMERALD);
        drawAttItem('Absent Days:', abs, margin + colW * 3, y, ROSE);

        y += 5.5;

        // Row 2
        drawAttItem('Half Days:', hd, margin, y, NAVY);
        drawAttItem('Paid Leave:', paidL, margin + colW, y, EMERALD);
        drawAttItem('Unpaid Leave:', unpaidL, margin + colW * 2, y, ROSE);
        drawAttItem('Weekly Off/Hol:', `${wOffs} / ${hols}`, margin + colW * 3, y, NAVY);
    ...
    if (lineItems && lineItems.length > 0) {
        lineItems.forEach(item => {
            ...
            if (item.source_type === 'incentive') {
                tableData.push([{ content: item.label, styles: { textColor: BLUE } }, fmt(amt)]);
            } else if (item.source_type === 'adjustment') {
                if (item.label.includes('+')) {
                    tableData.push([{ content: item.label, styles: { textColor: EMERALD } }, fmt(amt)]);
                } else {
                    deductionsData.push([{ content: item.label, styles: { textColor: ROSE } }, fmt(amt)]);
                }
    ...
    const totalDeductionsAmt = salary.deductions || 0;
    const grossEarnings = (salary.net_salary || 0) + totalDeductionsAmt;
    ...
    autoTable(doc, {
        ...
        body: deductionsData,
        foot: [['Total Deductions', fmt(totalDeductionsAmt)]],
        theme: 'grid',
