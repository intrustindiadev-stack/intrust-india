...
export const downloadPayslip = async ({ employee, salary, lineItems, attendanceStats: initialAttendanceStats }) => {
    ...
    if (attendanceStats) {
        ...
        const payableDays = Math.max(0, wkDays - abs - (hd * 0.5));
        ...
        const drawAttItem = (lbl, val, xPos, yPos, valColor = NAVY) => {
            ...
            doc.text(String(val), xPos + 24, yPos);
        };
        ...
        drawAttItem('Weekly Offs:', wOffs, margin + colW, y, NAVY);
        drawAttItem('Holidays:', hols, margin + colW * 2, y, NAVY);
        drawAttItem('Payable Days:', payableDays, margin + colW * 3, y, EMERALD);
