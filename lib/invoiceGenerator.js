...
export const generateOrderInvoice = async (params) => {
    ...
    const isRetailShopping = type === 'shopping' && order?.order_type !== 'wholesale';
    if (isRetailShopping && !allowIneligible && !isOrderInvoiceEligible(order)) {
    ...
    const paymentStatusUpper = (order.payment_status || (type === 'giftcard' || type === 'nfc' || type === 'wholesale' ? 'paid' : '')).toUpperCase();
    ...
    const invoiceTitle = type === 'wholesale' ? 'TAX INVOICE (B2B)' : 'TAX INVOICE';
    let y = await drawHeader(doc, pageWidth, invoiceTitle, invoiceNumber, invoiceDate, null, null, headerStatus);
    ...
    const paymentMethodLabel = paymentMethodMap[order.payment_method?.toLowerCase()] || (order.payment_method ? order.payment_method.toUpperCase() : (type === 'giftcard' || type === 'nfc' ? 'Online Payment' : (type === 'wholesale' ? 'InTrust Wallet' : 'N/A')));
    const paymentStatusLabel = (order.payment_status || (type === 'giftcard' || type === 'nfc' || type === 'wholesale' ? 'PAID' : 'PAID')).toUpperCase();
    const deliveryStatusLabel = (order.delivery_status || (type === 'giftcard' ? 'Delivered (Digital)' : (type === 'nfc' ? 'Dispatched' : (type === 'wholesale' ? 'Stock Added' : 'Packed')))).toUpperCase();
