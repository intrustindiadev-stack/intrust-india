...
/**
 * Helper: GST Resolution
 */
const isUnregistered = (val) => !val || ['unregistered', 'n/a', 'not provided', '-', 'null', 'undefined'].includes(val.toString().toLowerCase().trim());
...
export const generateManualInvoice = (data) => {
    ...
    const gstinValue = isUnregistered(sellerGst) ? PLATFORM_CONFIG.business.gstin : sellerGst;
    const gstinLabel = isUnregistered(sellerGst) ? "InTrust GST" : "GSTIN";

    if (gstinValue) {
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(NAVY_BLUE.r, NAVY_BLUE.g, NAVY_BLUE.b);
        doc.text(`${gstinLabel}: ${gstinValue}`, margin, addrEndY + 2);
    }
    ...
    addRow('SGST Output', fmt(sgst));
    addRow('CGST Output', fmt(cgst));
...
export const generateOrderInvoice = (params) => {
    ...
    if (gstinValue) {
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(NAVY_BLUE.r, NAVY_BLUE.g, NAVY_BLUE.b);
        doc.text(`${gstinLabel}: ${gstinValue}`, margin, currentY + 20);
    }
    ...
    if (totalGst > 0) {
        ...
        addRow('SGST (9%)', fmt(sgst));
        addRow('CGST (9%)', fmt(cgst));
    }
    doc.line(totalsX, finalY - 2, pageWidth - margin, finalY - 2);
    finalY += 4;
