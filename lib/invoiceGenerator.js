...
export const generateManualInvoice = (data) => {
    ...
    // If the remaining space is not enough for the totals and terms sections, add a new page
    if (tableEndY + 95 > 280) {
        doc.addPage();
        tableEndY = 20;
    }
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.setTextColor(255, 255, 255);
        doc.setFontSize(6);
        doc.text('Powered by Intrust Finance India Private Limited.', pageWidth / 2, 293, { align: 'center' });
    }
...
export const generateOrderInvoice = (params) => {
    ...
    if (type === 'giftcard') {
        ...
        processedItems = [{
            ...
            hsn: '9971',
            qty: '1',
            unitPrice: `Rs. ${fmt(base)}`,
            gstPct: '0%',
            gstAmt: `Rs. ${fmt(gst)}`,
            total: `Rs. ${fmt(grandTotal_gc)}`,
        }];
        subtotal = base;
        totalGst = gst;
        grandTotal = grandTotal_gc;
    } else if (type === 'nfc') {
        // NFC Order: Breakdown by Card and Delivery
        const salePrice = (order.sale_price_paise || 0) / 100;
        const DELIVERY_FEE = 50; 
        const cardTotal = salePrice - DELIVERY_FEE;
        const baseAmount = cardTotal;
        const gstAmount = Math.round(cardTotal * 0.18 * 100) / 100;
        const cardGrandTotal = baseAmount + gstAmount + DELIVERY_FEE;
        ...
        processedItems.push({
            sNo: '1',
            name: 'InTrust Premium NFC Card (Elite)',
            hsn: '9971',
            qty: '1',
            unitPrice: `Rs. ${fmt(baseAmount)}`,
            gstPct: '18%',
            gstAmt: `Rs. ${fmt(gstAmount)}`,
            total: `Rs. ${fmt(baseAmount + gstAmount)}`,
        });

        processedItems.push({
            sNo: '2',
            name: 'Secure Courier & Handling',
            hsn: '9971',
            qty: '1',
            unitPrice: `Rs. ${fmt(DELIVERY_FEE)}`,
            gstPct: '0%',
            gstAmt: 'Rs. 0.00',
            total: `Rs. ${fmt(DELIVERY_FEE)}`,
        });
    } else {
        ...
        items.forEach((item, idx) => {
            ...
            processedItems.push({
                ...
                unitPrice: `Rs. ${fmt(baseUnitPrice / 100)}`,
                gstPct: `${gstRate}%`,
                gstAmt: `Rs. ${fmt(gstAmount / 100)}`,
                total: `Rs. ${fmt(itemTotal / 100)}`,
            });
        });
    ...
    // Add delivery fee if applicable (Skip if NFC as it's already included in the nfc block above)
    const deliveryFee = (order.delivery_fee_paise || 0) / 100;
    if (deliveryFee > 0 && type !== 'nfc') {
        processedItems.push({
