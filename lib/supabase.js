 ...
 */
export async function signInWithOTP(phone, flow = null) {
    try {
        const response = await fetch('/api/auth/send-otp', {
            ...
            body: JSON.stringify({ phone, flow }),
        });
        ...
        try {
            ...
            console.error('[send-otp] Non-JSON response:', text.substring(0, 500));
...
export async function verifyOTP(phone, otp, full_name = null) {
    ...
    try {
        ...
        return {
            data: { user: data.user, role: data.role ?? null, is_suspended: data.is_suspended ?? false },
            error: null
        };
    } catch (error) {
