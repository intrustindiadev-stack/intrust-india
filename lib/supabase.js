...
/**
 * Sign in with OTP
 * Calls the custom API route to send OTP via SMS (default) or WhatsApp.
 * @param {string} phone - The phone number to send OTP to.
 * @param {'sms'|'whatsapp'} [channel='sms'] - Delivery channel.
 */
export async function signInWithOTP(phone, channel = 'sms') {
    try {
        ...
        try {
            ...
            console.error('[request-otp] Non-JSON response:', text.substring(0, 500));
            return { error: { message: `Server error (${response.status}). Check server logs.` } };
        }

        if (!response.ok) {
            return { error: { message: data.error || 'Failed to send OTP', retry_after: data.retry_after } };
        }
...
export async function verifyOTP(phone, otp, full_name = null) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return { data, error: { message: data.error || 'Invalid OTP' } };
        }
