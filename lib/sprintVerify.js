...
class SprintVerifyService {
    ...
    async verifyPAN(panNumber) {
        ...
        if (process.env.SPRINT_VERIFY_DEV_BYPASS === 'true') {
            console.log(`[SprintVerify - DEV BYPASS] Automatically verifying PAN: ${panNumber}`);
            return {
                valid: true,
        ...
        // Production endpoint: /verification/pan_advanced
        // Reverting to pan_verify as pan_advanced is currently failing
        const endpoint = '/verification/pan_verify';
    ...
    async _callVerificationAPI(endpointSuffix, payload, dataMapper) {
        ...
        const makeRequest = async () => {
            ...
            console.log(`[SprintVerify] [${isProduction ? 'LIVE' : 'UAT'}] Calling ${url}`, fullPayload);
