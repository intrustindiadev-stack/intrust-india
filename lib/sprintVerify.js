...
class SprintVerifyService {
    constructor() {
        ...
        if (process.env.NODE_ENV === 'production') {
            if (!this.jwtKey || !this.authorizedKey || !this.partnerId) {
                console.error('[SprintVerify] FATAL ERROR: Missing required environment variables (SPRINT_VERIFY_JWT_KEY, SPRINT_VERIFY_AUTHORIZED_KEY, or SPRINT_VERIFY_PARTNER_ID). Verification system will fail.');
                // In production, we want to know immediately if the configuration is broken
            }
            if (process.env.SPRINT_VERIFY_ENV !== 'LIVE') {
                console.warn('[SprintVerify] WARNING: Running in production but SPRINT_VERIFY_ENV is not LIVE. Using UAT endpoints.');
            }
        } else {
    ...
    async verifyPAN(panNumber) {
        // Bypass for local development testing to avoid static IP restrictions
        // NEVER allow this in production — the env var must not be set in prod.
        if (process.env.SPRINT_VERIFY_DEV_BYPASS === 'true') {
            if (process.env.NODE_ENV === 'production') {
                throw new Error('[SprintVerify] Dev bypass attempted in production — blocked.');
            } else {
                // Redact PAN from logs; only log last 4 chars
                const redacted = panNumber ? `****${panNumber.slice(-4)}` : '(empty)';
                console.log(`[SprintVerify - DEV BYPASS] Automatically verifying PAN: ${redacted}`);
                return {
                    valid: true,
                    message: 'Verified Successfully (Development Bypass)',
                    data: {
                        ...
                        gender: 'M',
                        ref_id: 'DEV_TEST_REF_' + Date.now(),
                        raw: { note: 'Generated via local dev bypass' }
                    }
                };
            }
        }

        // Endpoint is config-driven so UAT vs production can be switched via env var:
        //   UAT:        SPRINT_VERIFY_PAN_ENDPOINT=/verification/pan_verify  (default)
        //   Production: SPRINT_VERIFY_PAN_ENDPOINT=/verification/pan_advanced
        ...
        return this._callVerificationAPI(endpoint, { pannumber: panNumber }, (data) => ({
            ...
            dob: data.dob,
            // Fallback to different possible keys that the API might return
            father_name: data.father_name || data.fatherName || (data.raw_data ? data.raw_data.father_name : null),
            gender: data.gender,
    ...
    async _callVerificationAPI(endpointSuffix, payload, dataMapper) {
        ...
        const makeRequest = async () => {
            ...
            const isProduction = process.env.SPRINT_VERIFY_ENV === 'LIVE' || url.includes('api.paysprint.in') || url.includes('verifya2z.com');
            // Redact sensitive fields (PAN, account numbers) before logging
            const safePayload = Object.fromEntries(
                Object.entries(fullPayload).map(([k, v]) =>
                    ['pannumber', 'account', 'pan'].includes(k.toLowerCase())
                        ? [k, typeof v === 'string' ? `****${v.slice(-4)}` : '***']
                        : [k, v]
                )
            );
            console.log(`[SprintVerify] [${isProduction ? 'LIVE' : 'UAT'}] Calling ${url}`, safePayload);
        ...
        try {
            ...
            if (process.env.NODE_ENV !== 'production') {
                console.log(`\n\n[SprintVerify] ====== RAW API RESPONSE FROM ${endpointSuffix} ======`);
            ...
            const statusOk = data.status === true || String(data.status).toLowerCase() === 'true' || data.status == 1;
            const codeOk = data.response_code == 1 || data.statuscode == 200 || data.statuscode == 201;
            const isSuccess = statusOk && codeOk;
