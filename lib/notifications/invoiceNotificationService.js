...
export async function sendInvoiceNotification({
    ...
    // 3. Resolve Recipient
    // For WHATSAPP, strictly resolve from trusted invoice.customer_snapshot.phone
    // to prevent arbitrary recipient injection or spam.
    const resolvedRecipient = channel === 'WHATSAPP'
        ? (invoice.customer_snapshot?.phone ? String(invoice.customer_snapshot.phone).trim() : null)
    ...
    if (channel === 'WHATSAPP') {
        ...
        if (digits.length < 10) {
            const idempotencyKey = `${invoiceId}:${notificationType}:${channel}:INVALID_PHONE`;
            try {
                await supabaseAdmin.from('invoice_notifications').upsert({
                    invoice_id: invoiceId,
                    notification_type: notificationType,
                    ...
                    status: 'SKIPPED',
                    idempotency_key: idempotencyKey,
                    error_message: 'Customer phone number is invalid (must have at least 10 digits).',
                    metadata
                }, { onConflict: 'idempotency_key' });
            } catch (_) {}
            ...
            return {
                success: false,
                skipped: true,
                reason: 'Customer phone number is invalid (must have at least 10 digits).'
            };
        }
    }
    ...
    if (channel === 'EMAIL') {
        ...
        // Live Send via Centralized Transactional Email Service
        try {
            ...
            // If missing SMTP in dev/test environment, handle simulated fallback gracefully
            if (sendErr?.code === 'ERR_MISSING_SMTP_CONFIG') {
    ...
    if (channel === 'WHATSAPP') {
        // Format payable amount in rupees
        const amountDuePaise = Math.max(0, (invoice.grand_total_paise || 0) - (invoice.amount_paid_paise || 0));
        const payablePaise = (invoice.status === 'PAID') 
            ? (invoice.grand_total_paise || 0) 
            : (amountDuePaise > 0 ? amountDuePaise : (invoice.grand_total_paise || 0));
        const payableAmountStr = ((payablePaise || 0) / 100).toLocaleString('en-IN', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
        });
        const customerName = invoice.customer_snapshot?.name || 'Customer';
        const invoiceNumber = invoice.invoice_number;
        const publicPaymentToken = invoice.public_payment_token;

        const components = INVOICE_NOTIFICATION_TEMPLATE.buildComponents(
            customerName,
            invoiceNumber,
            payableAmountStr,
            publicPaymentToken
        ...
        if (!process.env.OMNIFLOW_BASE_URL || !process.env.OMNIFLOW_API_TOKEN) {
            // OmniFlow credentials not configured (e.g. in test or dev environment)
            console.warn(`[Invoice Notification] OmniFlow credentials not configured. WhatsApp notification ${notificationType} simulated for ${resolvedRecipient}.`);
            const simulatedMsgId = `simulated-wa-${Date.now()}`;
            ...
            await supabaseAdmin.from('invoice_events').insert({
                ...
                actor_id: actorId,
                event_type: 'INVOICE_NOTIFICATION_SENT',
                description: `Notification (${notificationType}) sent to ${maskRecipient(resolvedRecipient)} via WHATSAPP (Simulated).`,
                metadata: {
                    notification_type: notificationType,
                    channel: 'WHATSAPP',
                    recipient_masked: maskRecipient(resolvedRecipient),
        ...
        // Live Send via OmniFlow
        try {
            const sendResult = await sendTemplateMessage(
                resolvedRecipient,
                INVOICE_NOTIFICATION_TEMPLATE.name,
                INVOICE_NOTIFICATION_TEMPLATE.language,
                components
            );
            ...
            await supabaseAdmin.from('invoice_events').insert({
                ...
                actor_id: actorId,
                event_type: 'INVOICE_NOTIFICATION_SENT',
                description: `Notification (${notificationType}) sent to ${maskRecipient(resolvedRecipient)} via WHATSAPP.`,
                metadata: {
                    notification_type: notificationType,
                    channel: 'WHATSAPP',
                    recipient_masked: maskRecipient(resolvedRecipient),
            ...
            console.error(`[Invoice Notification] WhatsApp delivery failed:`, sendErr.message);
            ...
            if (notifId) {
                await supabaseAdmin
                    ...
                    .update({
                        status: 'FAILED',
                        failed_at: new Date().toISOString(),
                        error_code: sendErr.code || 'OMNIFLOW_ERROR',
                        error_message: sendErr.message || 'WhatsApp delivery failed'
                    })
                    .eq('id', notifId);
            }
            ...
            await supabaseAdmin.from('invoice_events').insert({
                ...
                actor_id: actorId,
                event_type: 'INVOICE_NOTIFICATION_FAILED',
                description: `WhatsApp delivery (${notificationType}) failed for ${maskRecipient(resolvedRecipient)}: ${sendErr.message}`,
                metadata: {
                    notification_type: notificationType,
                    channel: 'WHATSAPP',
                    error: sendErr.message
