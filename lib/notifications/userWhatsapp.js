...
// ─── Rotating daily tips (legacy, retained for reference) ───────────────────
...
export async function broadcastMorningGreeting() {
  ...
  const writeBroadcastAudit = async (status, summary) => {
    try {
      const { error: insertError } = await adminClient.from('whatsapp_message_logs').insert({
        user_id: null,
        phone_hash: SYSTEM_BROADCAST_PHONE_HASH,
        direction: 'outbound',
        message_type: 'template',
        channel: 'whatsapp',
        audience: 'customer',
      ...
      if (insertError) {
        console.error('[gm-broadcast-audit] DB insert error:', insertError.message);
      }
    } catch (err) {
      console.error('[gm-broadcast-audit] Failed to write broadcast audit:', err.message || err);
    }
  };

  // ── 0. Fetch today's quote (Admin scheduled quote takes priority) ─────────
  let quoteText = null;
  let quoteId = null;
  let isScheduledQuote = false;
  ...
  if (todayQuote?.quote_text) {
    quoteText = todayQuote.quote_text;
    quoteId = todayQuote.id;
    isScheduledQuote = true;
    console.log(`[gm-broadcast] Admin-scheduled quote found (id=${quoteId}): "${quoteText.slice(0, 60)}…"`);
  } else {
    quoteText = getFallbackDailyQuote(todayDateISO);
    quoteId = 'fallback';
    console.log(`[gm-broadcast] No admin quote scheduled for ${todayDateISO}. Using approved fallback quote: "${quoteText.slice(0, 60)}…"`);
  }
  ...
  if (!bindings || bindings.length === 0) {
    await writeBroadcastAudit('skipped', `no_opted_in_recipients quote_id:${quoteId}`);
    return { sent: 0, skipped: 0, failed: 0, total: 0, quote_id: quoteId };
  }
  ...
  if (merchantIds.length > 0) {
    ...
    if (merchantError) {
      ...
      for (const m of merchants) {
        nameMap[m.user_id] = nameMap[m.user_id] || (m.business_name || '').split(' ')[0]?.trim() || 'there';
      }
    }
  }
  ...
  // ── 4. Mark the scheduled quote as sent (if admin quote was used) ─────────
  if (isScheduledQuote && quoteId) {
  ...
  await writeBroadcastAudit(
    'sent',
    `done — sent:${sent} skipped:${skipped} failed:${failed} total:${uniqueBindings.length} quote_id:${quoteId}`
  );

  return { sent, skipped, failed, total: uniqueBindings.length, quote_id: quoteId };
}
...
export async function broadcastEveningGreeting() {
  ...
  const writeBroadcastAudit = async (status, summary) => {
    try {
      ...
      if (insertError) {
        console.error('[ge-broadcast-audit] DB insert error:', insertError.message);
      }
    } catch (err) {
      console.error('[ge-broadcast-audit] Failed to write broadcast audit:', err.message || err);
    }
  };
  ...
  try {
    ...
    try {
      ...
      await adminClient.from('whatsapp_message_logs').insert({
        ...
        status: 'sent',
        wamid: res?.messageId ?? null,
        content_preview: dedupeTag || `[template:${template.name}]`,
      ...
      try {
        await adminClient.from('whatsapp_message_logs').insert({
          ...
          content_preview: '[FAILED] ' + (dedupeTag || `[template:${template.name}]`) + ' :: ' + sendError.message.slice(0, 150),
          template_name: template.name,
        })
      } catch (logErr) {}
      throw sendError
...
export async function notifyCustomerOrderStatus({ userId, orderId, newStatus }) {
  ...
  return _dispatchCustomerUtilityWhatsApp({
    ...
    dedupeTag: `[template:intrust_order_status_v1:${orderId}:${newStatus}]`,
    dedupeWindowMs: 60 * 1000 // 1 min dedupe window for network retries
  })
}
