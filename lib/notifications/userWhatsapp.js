...
export async function broadcastMorningGreeting() {
  ...
  // ── 2. Batch-fetch first names from user_profiles by user_id list ────────
  ...
  if (userIds.length > 0) {
    ...
    if (profileError) {
      // Non-fatal: log a warning but continue — all senders will fall back to 'there'
      console.warn('[gm-broadcast] user_profiles fetch error (will use fallback name):', profileError.message);
    } else if (profiles) {
 ...
 * @param {string} opts.todayTag     - dedupe tag scoped to today's date
 */
async function _sendEveningGreeting({ adminClient, userId, phone, firstName, todayTag }) {
  // 1. Dedupe — skip if already sent today
  const { data: existing } = await adminClient
    .from('whatsapp_message_logs')
    .select('id')
...
export async function broadcastEveningGreeting() {
  ...
  // ── 2. Batch-fetch first names from user_profiles ───────────────────────────
  const userIds = bindings.map(b => b.user_id).filter(Boolean);
  let nameMap = {}; // user_id → firstName
  ...
  if (userIds.length > 0) {
    ...
    if (profileError) {
      console.warn('[ge-broadcast] user_profiles fetch error (will use fallback name):', profileError.message);
    } else if (profiles) {
      for (const p of profiles) {
        const firstName = (p.full_name || '').split(' ')[0]?.trim() || 'there';
        nameMap[p.id] = firstName;
