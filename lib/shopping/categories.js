...
// Presentational fallback categories for initial UI rendering.
// NOTE: Canonical taxonomy (categories, sub-categories, schemas) is defined in @/lib/constants/categories.js.
...
export const FALLBACK_CATEGORIES = [
    ...
    { id: 'cat-fashion', name: 'Fashion', slug: 'fashion', tag: 'Trending' },
    { id: 'cat-home', name: 'Home', slug: 'home', tag: 'Living' },
    { id: 'cat-beauty', name: 'Beauty', slug: 'beauty', tag: 'Care' },
    { id: 'cat-sports', name: 'Sports', slug: 'sports', tag: 'Fitness' },
];

/**
 * Returns a high-quality, category and product-aware image.
 * Solves the issue where denim jeans or shirts showed generic headphones photos.
 */
export function getProductFallbackImage(product) {
    ...
    const title = (product.title || product.custom_title || '').toLowerCase();
    const category = (product.category || '').toLowerCase();
    const subCategory = (product.sub_category || '').toLowerCase();
    const combined = `${title} ${category} ${subCategory}`;

    // If an image already exists, verify it doesn't mistakenly show the generic yellow headphones photo for fashion/groceries
    const isGenericHeadphonesPhoto = rawImage && rawImage.includes('photo-1505740420928-5e560c06d30e');
    const isAudioProduct = combined.includes('headphone') || combined.includes('earphone') || combined.includes('audio') || combined.includes('sound') || combined.includes('earbud');

    if (rawImage && (!isGenericHeadphonesPhoto || isAudioProduct)) {
        return rawImage;
    }

    // Category and Keyword-Aware Curated Images
    if (combined.includes('jean') || combined.includes('denim') || combined.includes('pant') || combined.includes('trouser')) {
        return 'https://images.unsplash.com/photo-1542272604-787c3835535d?w=600&auto=format&fit=crop&q=80';
    }
    if (combined.includes('t-shirt') || combined.includes('tshirt') || combined.includes('tee') || combined.includes('cotton t-shirt')) {
        return 'https://images.unsplash.com/photo-1521572267360-ee0c2909d518?w=600&auto=format&fit=crop&q=80';
    }
    if (combined.includes('shirt') || combined.includes('kurta') || combined.includes('hoodie') || combined.includes('jacket') || combined.includes('cloth') || combined.includes('fashion') || combined.includes('wear') || combined.includes('apparel')) {
    ...
    if (combined.includes('headphone') || combined.includes('earphone') || combined.includes('audio') || combined.includes('sound') || combined.includes('earbud')) {
        return 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=600&auto=format&fit=crop&q=80';
    }
    if (combined.includes('phone') || combined.includes('mobile') || combined.includes('smartphone') || combined.includes('tablet') || combined.includes('ipad') || combined.includes('iphone')) {
        return 'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=600&auto=format&fit=crop&q=80';
    }
    if (combined.includes('grocer') || combined.includes('food') || combined.includes('oil') || combined.includes('rice') || combined.includes('dal') || combined.includes('tea') || combined.includes('coffee') || combined.includes('spice') || combined.includes('snack') || combined.includes('biscuit')) {
        return 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=600&auto=format&fit=crop&q=80';
    }
    if (combined.includes('home') || combined.includes('kitchen') || combined.includes('decor') || combined.includes('lamp') || combined.includes('bed')) {
        return 'https://images.unsplash.com/photo-1583847268964-b28dc8f51f92?w=600&auto=format&fit=crop&q=80';
    }
    if (combined.includes('beauty') || combined.includes('skincare') || combined.includes('cosmetic') || combined.includes('perfume') || combined.includes('cream')) {
        return 'https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?w=600&auto=format&fit=crop&q=80';
    }
    if (combined.includes('elect') || combined.includes('gadget') || combined.includes('laptop') || combined.includes('keyboard') || combined.includes('mouse')) {
        return 'https://images.unsplash.com/photo-1550009158-9ebf69173e03?w=600&auto=format&fit=crop&q=80';
    }
    if (combined.includes('solar') || combined.includes('panel') || combined.includes('inverter') || combined.includes('battery')) {
