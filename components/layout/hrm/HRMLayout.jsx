...
export default function HRMLayout({ children, userProfile }) {
    ...
    const userName = userProfile?.full_name || 'HR Admin';
    
    // HRM uses Emerald colors
    const accentText = 'text-emerald-600';
    const accentBg = 'bg-emerald-600';
    const accentShadow = 'shadow-emerald-500/20';

    return (
        <div className="min-h-screen bg-[#F8FAFC] transition-colors duration-300">
            {/* Sidebar */}
            <HRMSidebar isOpen={sidebarOpen} setIsOpen={setSidebarOpen} userProfile={userProfile} />
            <HRMBottomNav isSidebarOpen={sidebarOpen} userProfile={userProfile} />
                ...
                <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-gray-200 shadow-sm transition-colors duration-300">
                    <div className="flex items-center justify-between px-4 sm:px-6 py-4">
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                            {/* Notifications */}
                            <NotificationBell apiPath="/api/hrm/notifications" />
