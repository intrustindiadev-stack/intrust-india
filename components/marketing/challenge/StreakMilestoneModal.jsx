...
export default function StreakMilestoneModal({
    ...
    const effectiveStreak = Math.max(1, Number(streak || 1));
                ...
                >
                        ...
                        <motion.div
                            ...
                            className={`w-24 h-24 rounded-3xl flex items-center justify-center shadow-xl ${
                                hasMilestone
                                    ...
                                    : 'bg-gradient-to-tr from-amber-500/20 via-orange-500/25 to-rose-500/20 text-orange-500 border border-orange-400/40 shadow-orange-500/20'
                            }`}
                        >
                            {hasMilestone ? <Trophy size={48} className="animate-bounce" /> : <Flame size={48} className="fill-orange-500 text-orange-500 animate-pulse" />}
                        </motion.div>
                        {hasMilestone && (
                    ...
                    <div>
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-500/10 text-orange-600 dark:text-orange-400 text-xs font-black uppercase tracking-wider mb-2 border border-orange-500/20">
                            ...
                            <span>{effectiveStreak}-Day Challenge Streak!</span>
                        </div>
                        <h2 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight">
                            {hasMilestone ? 'Streak Milestone Unlocked!' : effectiveStreak === 1 ? 'Streak Started!' : 'Streak Extended!'}
                        </h2>
                        {badge && (
                    ...
                    <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/80 space-y-2 text-left">
                        <div className="flex items-center justify-between text-xs font-bold text-slate-600 dark:text-slate-300">
                            <span>Daily Challenge Cashback</span>
                        ...
                        {hasMilestone && (
                            <div className="flex items-center justify-between text-xs font-black text-orange-600 dark:text-orange-400">
                                <span className="flex items-center gap-1">
                                    <Sparkles size={13} /> {effectiveStreak}-Day Streak Bonus
                                </span>
