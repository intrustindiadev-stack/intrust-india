...
export default function GuideInfoButton({ pageKey, scope = 'marketing', className = '' }) {
    ...
    const { guideKey, openGuide, closeGuide } = authContext;
    ...
    const isOpen = localOpen || (guideKey === `${scope}:${pageKey}`);

    const handleOpen = () => {
        setLocalOpen(true);
        if (typeof openGuide === 'function') {
            try { openGuide(`${scope}:${pageKey}`); } catch (_) {}
        }
    };

    const handleClose = () => {
        setLocalOpen(false);
        if (typeof closeGuide === 'function') {
    ...
    return (
        <>
            ...
            <GuideInfoModal guide={guide} open={isOpen} onClose={handleClose} />
        </>
    );
}
