...
import { Plus, Minus, Check, Heart, Zap, Star, Eye, ShoppingBag } from 'lucide-react';
...
function ProductCardV2({ 
    ...
    const oos = item?.outOfStock ?? isStorefrontItemOOS(item);
    ...
    const rawSellingPrice = item?.is_platform_product
        ? ((product?.platform_price_paise ?? product?.suggested_retail_price_paise) || item?.retail_price_paise || 0)
        : (item?.retail_price_paise || item?.price_paise || product?.platform_price_paise || product?.suggested_retail_price_paise || 0);

    const sellingPrice = rawSellingPrice / 100;
    ...
    const displayImage = item?.image || getProductFallbackImage(product || item);
    const title = item?.custom_title || product?.title || item?.title || 'Product';
    const category = item?.category || product?.category || (item?.shopping_categories?.name) || 'General';
        ...
        >
            <div>
                ...
                <div className="flex items-center gap-1 mb-0.5">
                    <p className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider truncate">
                        {item.merchants?.business_name || category || 'InTrust Direct'}
                    </p>
                ...
                >
                    ...
                    <div className="flex items-center justify-between gap-1.5 mt-1">
                        <span className={`text-[10px] font-extrabold uppercase tracking-wider truncate min-w-0 ${
                            ...
                            {item.sub_category || product?.sub_category || category}
                        </span>
                        <div className="flex items-center gap-0.5 shrink-0 text-[10px] font-black text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 px-1.5 py-0.5 rounded-md">
                            ...
                            <span>{product?.rating || item?.rating || '4.8'}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="w-full relative z-10" onClick={e => e.stopPropagation()}>
                    {oos ? (
                        ...
                        <button
                            type="button"
                            onClick={handleOpenPdp}
                            className="w-full h-8 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-600 text-slate-700 dark:text-slate-300 text-xs font-bold transition-all flex items-center justify-center gap-1"
                        >
