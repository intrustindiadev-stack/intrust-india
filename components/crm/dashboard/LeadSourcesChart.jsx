...
export default function LeadSourcesChart({ 
    ...
    return (
        ...
        >
            <div className="absolute -top-32 -right-32 w-64 h-64 bg-pink-500/5 dark:bg-pink-500/10 rounded-full blur-3xl group-hover:bg-pink-500/10 dark:group-hover:bg-pink-500/20 transition-colors" />
            
            <div className="flex items-center justify-between mb-8 relative z-10">
                <div>
                    <h2 className="text-lg font-black text-gray-900 dark:text-white flex items-center gap-2">
                        {Icon && <Icon size={20} className="text-pink-500 dark:text-pink-400" />} {title}
                    </h2>
                    {subtitle && <p className="text-xs text-gray-500 dark:text-pink-400/70 font-bold tracking-wide mt-1 uppercase">{subtitle}</p>}
                </div>
            </div>
